...
seconds = float(video['duration'])
assert abs(seconds - (60 if seconds > 10 else 5)) < 1/30
pcm = np.frombuffer(subprocess.check_output(['ffmpeg', '-v', 'error', '-i', path, '-vn', '-f', 'f32le', '-ar', '48000', '-ac', '1', '-']), dtype='<f4')
levels = {}
for start in ([0.5, 2.5, 3.5, 4.5, 6.5, 21.5] if seconds > 10 else [0.5, 2.5, 3.5, 4.5]):
...
assert levels[6.5 if seconds > 10 else 2.5][440] > .03 and levels[0.5][440] < .01 and levels[21.5 if seconds > 10 else 4.5][440] < .01, levels
