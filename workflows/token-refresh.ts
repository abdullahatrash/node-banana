...
async function doRefresh(accountId: string): Promise<RefreshResult> {
  ...
  const {
    claimSocialTokenRefreshLease,
    getSocialAccountById,
    markRequiresReauth,
    releaseSocialTokenRefreshLease,
  ...
  const lease = await claimSocialTokenRefreshLease({
    workspaceId: account.workspaceId,
    socialAccountId: accountId,
    claimedBy: `token-refresh:${accountId}`,
  });
  ...
  try {
    ...
    await releaseSocialTokenRefreshLease({
      workspaceId: account.workspaceId,
      socialAccountId: accountId,
