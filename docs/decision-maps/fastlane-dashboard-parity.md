...
|---|---|---|---|
| Settings → Account (sidebar button; overlay) | **O:** profile/security tabs, email identities, connected login accounts, update profile, add email, delete account, sign out. **I:** identity-provider and account-lifecycle APIs. | Settings → Account consolidates bilingual Profile and Security tabs over Better Auth: display-name update, current-email approval plus new-email verification, linked login methods with last-method protection, configured OAuth linking, password change with other-session revocation, active-session review/revocation, and sign-out. Governed identity erasure adds an exact owned-Workspace preflight, links to transfer/export/closure, password or fresh-OAuth reauthentication, three explicit acknowledgements, atomic auth/session/membership deletion, direct-PII scrubbing, advertising-consent revocation, governance projection cancellation, a pseudonymous retained identity, bounded rewrite-proof completion evidence, and database anti-resurrection guards. | `existing` and bilingual. Active Workspace ownership fails closed until the Workspace is transferred or reaches terminal closure. Retained rights, audit, governance, and financial evidence follows its own lifecycle and remains linked only to the pseudonymous tombstone. |
| Settings → Billing | **O:** “Billing & Subscription” shell; detailed plan state did not render in the inspected trial. **I:** subscription/customer portal and entitlements. | `/pricing`, `/billing`, and Settings → Billing expose versioned Plan Definitions, trial eligibility, Workspace Subscription state, secure checkout, merchant portal, merchant-issued expiring invoice links, paid transaction history, refund/credit/dispute evidence, and role-gated administration. The authoritative commercial projection remains separate from External Provider Spend. | `existing` and bilingual. Production actions truthfully report unavailable until a configured replaceable Merchant-of-Record Adapter is present. Signed adjustments enter a normalized, digest-bound, leased recovery inbox before projection, so out-of-order transaction delivery, retries, worker crashes, and replay conflicts remain explicit instead of losing financial evidence. Credit-pack and exact-period subscription-allowance refunds or disputes proportionally claw back unspent credits, surface already-consumed liabilities, and block new managed execution until resolved. Fully refunded or disputed current subscription periods also carry a period-scoped execution hold, so unrelated credit buckets cannot bypass payment state and a later paid renewal is not blocked by an older hold. The app never fabricates tax documents. |
...
| Settings → Privacy | **O:** X Ads attribution toggle/status and irreversibility warning for already-sent events. | `/settings?section=privacy` exposes two independent personal, Workspace-scoped purposes. Product analytics supports explicit 30/90/365-day activation, immutable consent revisions, automatic expiry, content-free events, rotating pseudonyms, at-most-90-day retention, and revocation erasure. X Ads attribution uses its own expiring consent revisions, public notice and regional/access review gates, dedicated OAuth credentials, server-only Conversion API adapter, three allowlisted conversion types, pre-storage email hashing, stable provider deduplication, durable retries, terminal payload scrubbing, minimal delivery receipts, revocation cancellation, daily retention, and fail-closed operator readiness. Trial and signed merchant-purchase completions are connected producers. No browser advertising pixel is loaded. | `existing` and bilingual, with unavailable states rendered truthfully until every operator gate is configured. Product analytics never authorizes advertising attribution; customer X publishing credentials are not reused. Revocation stops new admission, scrubs queued local payloads, and rechecks consent before delivery, while copy explicitly states that accepted or already-in-flight external delivery cannot be recalled. |
...
| Settings → Notifications | **O:** email master switch, lead/activity/failed-post/disclosure-review digests. | `/settings?section=notifications` persists a validated per-person/per-Workspace delivery policy: social in-app/email channels, independent Arabic/English delivery language, cadence, quiet hours, optional digest categories and mute, plus explicit billing, Channel, publishing and credit email switches. A canonical cross-product projector derives security credential changes, Channel consent expiry/reconnection, Approval requests/decisions, Delivery and social publishing failures, low/exhausted credits, and refund/dispute transitions from append-only source evidence. The shell filters each event through the member's current exact permission, stores private recipient read state, snapshots authored Arabic/English delivery payloads, and uses a leased idempotent email outbox with explicit suppression, deterministic failure, and ambiguous-outcome states. | `existing` and bilingual for the observed surface plus stronger operational coverage. Security-change email remains mandatory for verified authorized managers; every optional email category is independently reversible. Projection precedes dispatch, source references are replay-safe, credit warnings are episode-based, and permission revocation suppresses list/read/email immediately without deleting evidence. |
