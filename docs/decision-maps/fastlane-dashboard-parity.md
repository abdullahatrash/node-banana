...
| Settings → Notifications | **O:** email master switch, lead/activity/failed-post/disclosure-review digests. | `/settings?section=notifications` persists a validated per-person/per-Workspace publishing delivery policy: in-app and email channels, independent Arabic/English delivery language, daily/weekly cadence, IANA-timezone quiet hours, optional progress/success/channel categories, and reversible optional mute. Refund and dispute projections emit separate canonical cross-product notification events, fan out only to current billing readers, snapshot an authored Arabic/English delivery locale, retain per-recipient read state, and use a leased idempotent email outbox with explicit suppression, deterministic failure, and ambiguous-outcome states. The authenticated shell exposes these billing events without leaking them through the social event stream. | `existing` for social publishing and billing financial events, with authored Arabic and English renderers. Known publishing failures and Channel reauthorization bypass optional social suppression; billing remains visible in-app and has a separate explicit email switch. Security, consent-expiry, and approval producers/renderers remain for global notification parity. |
