...
| Settings → AI Credits | **O:** balance, Buy more, monthly reset vs non-expiring add-on credits, recent ledger activity. **I:** credit ledger, reservation, settlement. | Billing exposes total Generation Credits, separate expiring allowance/non-expiring purchased/referral buckets, credit packs, and visibly held reservations. The backend persists immutable ledger entries, fixed Managed Execution Quotes, allowance-first allocation, release/refund, and outcome-unknown holds alongside separate BYOK Usage evidence. | `existing` core; customer presentation still needs paginated recent ledger and quote history, but balance, bucket, purchase, reservation, and generation-admission debit semantics are live and bilingual. |
