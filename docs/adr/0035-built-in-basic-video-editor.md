...
The editor includes at least two independently controlled added audio layers: Music and Voiceover. These are distinct from any audio embedded in the main footage or influencer reaction; source-audio retention and mixing rules still need to be settled. The layout must give Music and Voiceover separate sections that can later accept AI-generated audio as well as the initial audio source flow, which remains to be specified.

AI music and voiceover generation is explicitly deferred. Suno and ElevenLabs were named as examples of possible future providers, not selected integrations. Reserve a place in the interaction design for generation within the corresponding audio section; keep the editing model based on audio media references rather than provider-specific controls. Initial delivery must not require provider connections or generation calls. Whether audio can be uploaded, selected from Workspace media, or recorded directly, and the timing, volume, looping, and ducking behavior remain open.

Workspace autosave and initial input support up to 1080p were agreed earlier. Final export execution was initially agreed as server-side background work, then reopened when operating-cost concerns prompted a first-principles review of the use case; browser versus server export remains under reconsideration. The earlier ten-clip, three-minute general-editor scope and remaining broad features (including automatic subtitles) must be reconciled with this narrower two-video workflow rather than silently carried into implementation. Music and Voiceover layers have been explicitly reconfirmed for the narrower workflow.
