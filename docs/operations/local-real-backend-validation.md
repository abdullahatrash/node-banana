...
| Referral cash payouts | `/refer-and-earn` and internal payout worker | Verified recipient in the configured payout gateway | A request locks exact rewards, looks up its stable idempotency key before submission, keeps ambiguous funds held, and releases or pays the ledger only from verified terminal evidence. Follow `referral-payouts.md`. | Real money only after the external payout gateway is explicitly configured |
...
| YouTube public trends | `/inspiration` | YouTube Data API key and reviewed policy URLs | Only metadata and source links are stored; public YouTube media is never treated as remixable. Topic-only original creation stays disabled unless written approval for the exact durable derivation and retention use is recorded and the separate operator gate is enabled. Follow `youtube-trend-discovery.md`. | Google API quota for discovery; Generation Credits only if an approved topic is later accepted for generation in Blitz |
