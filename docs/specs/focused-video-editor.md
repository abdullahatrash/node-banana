...
- Timeline interactions must update the actual preview while dragging. A visible ruler and draggable playhead scrub through Main and Secondary video; Secondary appears only during its scheduled interval. Dragging Main scrubs; dragging Secondary, Music, or Voiceover moves the whole track within Main's duration. Retained sections of one source move together. Edge trimming previews the edited boundary, and one drag remains one undo step even when the pointer pauses. Keyboard arrows provide frame steps, with Shift for one-second steps.
