...
describe("RTL layout source gate", () => {
  it("rejects physical spacing and border utilities through variants", () => {
    expect(findPhysicalDirectionalUtilities('className="pl-5 md:mr-2 border-l rounded-r-xl"')).toEqual([
      "pl-5",
      "md:mr-2",
      "border-l",
      "rounded-r-xl",
  ...
  it("accepts logical utilities and explicit physical-edge drawer contracts", () => {
    expect(findPhysicalDirectionalUtilities('className="ps-5 md:me-2 border-s rounded-e-xl"')).toEqual([]);
