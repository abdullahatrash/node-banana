...
function getEncryptionKey(): Buffer | null {
  const hex = process.env.SOCIAL_TOKEN_ENCRYPTION_KEY?.trim();
  if (!hex) return null;
...
/**
 * Encrypt a plaintext token for storage at rest.
...
export function encryptToken(plaintext: string): string {
  ...
  if (!key) {
    ...
    throw new Error(
      "SOCIAL_TOKEN_ENCRYPTION_KEY is required for token encryption.",
    );
  }
...
export function decryptToken(encrypted: string): string {
  ...
  if (!key) {
    throw new Error(
      "SOCIAL_TOKEN_ENCRYPTION_KEY is required for token decryption.",
    );
  }
