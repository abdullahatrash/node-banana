...
describe("operation projection adapters", () => {
  it("covers all required owning resources", () => {
    expect(OPERATION_PROJECTION_ADAPTERS.map((item) => item.kind)).toEqual([
      "workflow_run", "brand_ingestion", "governance_export", "governance_bulk", "workspace_import", "automation", "campaign_automation", "publishing_delivery", "persona_training",
    ]);
  ...
  it("normalizes aborted and unknown source states safely", () => {
    const adapter = getOperationProjectionAdapter("publishing-deliveries/v1")!;
  ...
  it("projects campaign occurrences without conflating runtime Automations", () => {
    const adapter = getOperationProjectionAdapter("product-campaign-occurrences/v1")!;
    expect(adapter.kind).toBe("campaign_automation");
    expect(adapter.project({ workspaceId: "w", resourceId: "r", state: "submitting", updatedAt: new Date() }).state).toBe("waiting_provider");
  });
});
