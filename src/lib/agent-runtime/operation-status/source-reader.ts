...
export async function readSourceOperationProjections(database: Db, workspaceId: string, pageSize = 200): Promise<ProjectedSourceOperation[]> {
  ...
  const [runs, brands, governance, automations, deliveries] = await Promise.all([
    readAll(pageSize, (cursor) => database.select({ workspaceId: workflowRuns.workspaceId, resourceId: workflowRuns.id, state: workflowRuns.state, updatedAt: workflowRuns.updatedAt, workflowId: workflowRuns.workflowId, principalId: workflowRuns.principalId, authorizationEvidenceRef: workflowRuns.authorizationEvidenceRef, reasonCode: workflowRuns.failureCode }).from(workflowRuns).where(and(eq(workflowRuns.workspaceId, workspaceId), before(workflowRuns.updatedAt, workflowRuns.id, cursor))).orderBy(desc(workflowRuns.updatedAt), desc(workflowRuns.id)).limit(pageSize)),
    readAll(pageSize, (cursor) => database.select({ workspaceId: brandAnalysisRuns.workspaceId, resourceId: brandAnalysisRuns.id, state: brandAnalysisRuns.status, stage: brandAnalysisRuns.stage, updatedAt: brandAnalysisRuns.updatedAt, sourceId: brandAnalysisRuns.sourceId, reasonCode: brandAnalysisRuns.errorCode }).from(brandAnalysisRuns).where(and(eq(brandAnalysisRuns.workspaceId, workspaceId), before(brandAnalysisRuns.updatedAt, brandAnalysisRuns.id, cursor))).orderBy(desc(brandAnalysisRuns.updatedAt), desc(brandAnalysisRuns.id)).limit(pageSize)),
  ...
  return [
    ...
    ...governance.map((row) => project(row.kind === "bulk_operation" ? "governance-bulk/v1" : row.kind === "workspace_import" ? "workspace-imports/v1" : "governance-exports/v1", row, { principalId: row.principalId, resourceVersion: row.resourceVersion, nextAction: "inspect_governance_resource" })),
