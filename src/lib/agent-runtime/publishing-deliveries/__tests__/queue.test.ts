...
describe("DurablePublishingDeliveryQueue", () => {
  ...
  it.each(["publish", "reconcile"] as const)(
    "starts a durable %s worker only for the exact Delivery generation",
    async (purpose) => {
  ...
  it("rejects a retargeted or malformed durable dispatch identity", async () => {
    ...
    await expect(
      queue.schedule({
        workspaceId: "workspace_1",
        deliveryId: "delivery_1",
        purpose: "tampered" as "publish",
        dedupeKey: "publishing-delivery:workspace_1:delivery_1:v1",
      }),
    ).rejects.toThrow("dispatch identity is invalid");
