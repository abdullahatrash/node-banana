...
describe("Workspace Data Region route enforcement", () => {
  ...
  it("admits each exact route in its own signed region", async () => {
    const repository = new InMemoryGovernanceRepository();
    const routes = Object.values(GOVERNANCE_REGION_ROUTES).map((route) => ({
      ...route,
      region: route.kind === "processing" ? "replicate-us" : "me-central-1",
    }));
    const body = {
      region: "me-central-1",
      verified: true,
    ...
    await repository.commit({
      receipt: { workspaceId: "workspace-a", capability: "test.seed@1", idempotencyKey: "seed-multi-region", requestDigest: canonicalDigest(body), result: {}, createdAt: now },
      mutations: [{ type: "create", expectedVersion: null, resource: { id: "active", workspaceId: "workspace-a", kind: "data_region_policy", version: 1, status: "active", body, createdByUserId: "owner-a", createdAt: now, updatedAt: now } }],
      audit: { schema: "workspace-audit-event/v1", id: "audit-seed-multi-region", workspaceId: "workspace-a", actor: { kind: "system", id: null }, capability: "test.seed@1", action: "seed", resource: null, outcome: "completed", redactedDetails: {}, occurredAt: now },
    });
    const service = new GovernanceRegionAdmissionService(repository);
    const processing = Object.values(GOVERNANCE_REGION_ROUTES).find((route) => route.kind === "processing")!;
    await expect(service.admit({ workspaceId: "workspace-a", ...processing, configuredRegion: "replicate-us", evaluatedAt: now }))
      .resolves.toMatchObject({ allowed: true, policyApplied: true });
    await expect(service.admit({ workspaceId: "workspace-a", ...processing, configuredRegion: "me-central-1", evaluatedAt: now }))
      .resolves.toEqual({ allowed: false, reason: "REGION_ROUTE_NOT_ALLOWLISTED" });
  });
});
