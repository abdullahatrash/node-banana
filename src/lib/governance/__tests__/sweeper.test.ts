...
describe("GovernanceRecoverySweep", () => {
  it("dispatches every durable queued or recoverable lifecycle through its leased worker", async () => {
    ...
    expect(summary).toEqual({ workspaces: 1, examined: 5, dispatched: 5, failed: 0, deadlinesAdvanced: 2, membershipProjection: { scanned: 1, succeeded: 1, retryPending: 0, deadLetter: 0 }, expiredSecretDeliveriesPurged: 3, nextCursor: null });
