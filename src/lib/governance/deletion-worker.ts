...
export class GovernanceDeletionWorker {
  ...
  async process(input: { workspaceId: string; deletionReceiptId: string }): Promise<void> {
    const found = await this.repository.getResource<{ resourceKind: string; resourceId: string; retentionClass: string; systems: string[]; outcomes: Record<string, GovernanceDeletionSystemOutcome>; lease?: DeletionLease | null; [key: string]: unknown }>({ workspaceId: input.workspaceId, kind: "deletion_receipt", id: input.deletionReceiptId });
    ...
    const next: GovernanceResource = { ...job, version: job.version + 1, status, body: { ...body, lease: null, outcomes, completedAt: status === "completed" ? now.toISOString() : null }, updatedAt: now };
  ...
  async recoverExpired(input: { workspaceId: string }): Promise<number> {
    const now = this.clock.now();
    const jobs = (await this.repository.listResources<{ lease?: DeletionLease | null }>({ workspaceId: input.workspaceId, kinds: ["deletion_receipt"] }))
  ...
  private async claim(job: GovernanceResource<{ lease?: DeletionLease | null; [key: string]: unknown }>) {
    const now = this.clock.now();
    ...
    const next = { ...job, version: job.version + 1, status: "running", body: { ...job.body, lease }, updatedAt: now };
