...
export const governanceCommandSchema: z.ZodType<GovernanceCommand> = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("record_deletion"), resourceKind: id, resourceId: id, retentionClass: z.enum(RETENTION_CLASSES), immediate: z.array(text.max(500)), delayed: z.array(text.max(500)), retained: z.array(text.max(500)), holdIds: z.array(id), stepUpToken: id }).strict(),
  ...
  z.object({ type: z.literal("preview_import"), source: text.max(300), sourceManifestDigest: digest, items: z.array(z.object({ kind: id, sourceId: id, destinationId: id.optional(), digest, transferable: z.boolean(), omissionReason: text.max(500).optional(), payload: z.record(z.string(), z.unknown()).optional() }).strict()).min(1) }).strict(),
