...
const schemas = {
  ...
  calendar_plan: z.object({ schema: z.literal("portable-calendar-plan/v2"), id, sourceChannelId: id, status: z.enum(["draft", "queued", "publishing", "published", "failed"]), kind: id, content: z.string().nullable(), stableMediaRefs: z.array(z.object({ type: z.enum(["image", "video", "audio", "document", "model3d", "workflow"]), sourceKind: z.enum(["studio_asset", "artifact"]).optional(), sourceAssetId: id, assetDigest: z.string().regex(/^sha256:[a-f0-9]{64}$/), order: z.number().int().nonnegative(), alt: z.string().max(2_000).optional() }).strict()).superRefine((refs, context) => {
