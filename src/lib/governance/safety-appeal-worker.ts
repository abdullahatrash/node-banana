...
interface SafetyAppealBody {
  decisionId: string;
  intentRef: string;
  outcome: string;
  currentRevalidationRequired: boolean;
  canResume: boolean;
  lease?: SafetyLease | null;
  [key: string]: unknown;
}
...
export class GovernanceSafetyAppealWorker {
  ...
  async process(input: { workspaceId: string; appealId: string }): Promise<void> {
    const found = await this.repository.getResource<SafetyAppealBody>({ workspaceId: input.workspaceId, kind: "safety_appeal", id: input.appealId });
  ...
  async recoverExpired(input: { workspaceId: string }): Promise<number> {
    const now = this.clock.now();
    const appeals = (await this.repository.listResources<SafetyAppealBody>({ workspaceId: input.workspaceId, kinds: ["safety_appeal"] }))
  ...
  private async claim(appeal: GovernanceResource<SafetyAppealBody>): Promise<GovernanceResource<SafetyAppealBody> | null> {
    const now = this.clock.now();
    ...
    const next: GovernanceResource<SafetyAppealBody> = { ...appeal, version: appeal.version + 1, status: "revalidation_running", body: { ...appeal.body, lease }, updatedAt: now };
