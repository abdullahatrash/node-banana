...
export class DrizzleGovernanceAuditFederation implements GovernanceAuditFederationPort {
  ...
  async list(input: { workspaceId: string; limit: number; before?: { occurredAt: Date; id: string } }) {
    ...
    const [agents, credentialSecurity, credentialEffects, credentialSpend, budgets, spendControls, usage, usageReceipts, quotas, artifacts, runs, automations, deliveries, social, diagnostics, support] = await Promise.all([
