...
describe("admitted generation release-flag boundary", () => {
  it("fails closed in production and evaluates the authoritative flag before provider access", () => {
    const source = fs.readFileSync(path.join(process.cwd(), "src/app/api/studio/model-routing/intents/[intentId]/execute/route.ts"), "utf8");
    ...
    expect(source.indexOf("evaluateReleaseFlag")).toBeLessThan(source.indexOf("const credential = await resolveDurableProviderKey"));
