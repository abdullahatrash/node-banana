...
export function overlayLayout(context: PaintContext, overlay: TextOverlay) {
  ...
  let lines: { text: string; direction: CanvasDirection }[] = [], lineHeight = 0, height = 0;
...
export function paintOverlay(context: PaintContext, layout: ReturnType<typeof overlayLayout>) {
  context.save();
