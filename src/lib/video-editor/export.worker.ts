...
type AudioCursor = {
  ...
  clip: Clip;
  offset: number;
  samples: ReturnType<AudioSampleSink["samples"]>;
  done: boolean;
  current: {
    pcm: Float32Array;
    channels: number;
...
async function mix(cursor: AudioCursor, pcm: Float32Array, start: number) {
  ...
  for (let j = 0; j < block; j++) {
    ...
    while (
      ...
      const item = await cursor.samples.next();
      cursor.done = Boolean(item.done);
      if (item.done) {
        cursor.current = null;
        break;
      }
      ...
      try {
        if (sample.sampleRate !== rate || sample.numberOfChannels > 2)
    ...
    const index = Math.min(
      current.frames - 1,
      Math.max(0, Math.floor((time - current.start) * rate + 1e-6)),
    );
    pcm[j * 2] += current.pcm[index * current.channels] * cursor.clip.gain;
    pcm[j * 2 + 1] +=
      current.pcm[index * current.channels + (current.channels === 2 ? 1 : 0)] *
      cursor.clip.gain;
  }
}
function draw(
  ...
  const width = rect.width * 1080,
    ...
    scale = Math.max(width / canvas.width, height / canvas.height),
    sw = width / scale,
    sh = height / scale;
  context.drawImage(
    canvas,
...
scope.onmessage = async ({ data }) => {
  ...
  try {
    ...
    if (
      ...
      !(await canEncodeAudio("aac", { sampleRate: rate, numberOfChannels: 2 }))
    )
      throw new Error("EDITOR_UNSUPPORTED");
    const parent = await (
      await navigator.storage.getDirectory()
    ).getDirectoryHandle(data.rootName, { create: true });
    const directory = await parent.getDirectoryHandle(data.attempt, {
      create: true,
    });
    ...
    const videos: Partial<
      Record<
        "main" | "secondary",
        ReturnType<CanvasSink["canvasesAtTimestamps"]>
      >
    > = {};
    ...
    if (composition.text?.text) {
      ...
      checkCancelled();
      const layout = overlayLayout(context, composition.text),
        textCanvas = new OffscreenCanvas(layout.width, layout.height);
    ...
    const fileHandle = await directory.getFileHandle("video.mp4", {
      create: true,
    });
    output = new Output({
    ...
    for (let i = 0; i < count; i++) {
      ...
      for (const cursor of cursors) {
        affectedRole = cursor.role;
        await mix(cursor, pcm, time);
      }
      affectedRole = null;
      for (let sample = 0; sample < pcm.length; sample++)
        pcm[sample] = Math.max(-1, Math.min(1, pcm[sample]));
    ...
    errorCode = cancelled
      ...
      : error instanceof DOMException && error.name === "QuotaExceededError"
        ? "EDITOR_STORAGE_FULL"
        : error instanceof Error && error.message.startsWith("EDITOR_")
          ? error.message
          : "EDITOR_EXPORT_FAILED";
  } finally {
    await Promise.allSettled([
      ...frameIterators.map((iterator) => iterator.return(undefined)),
      ...cursors.map((cursor) => cursor.samples.return(undefined)),
    ]);
    for (const input of inputs) input.dispose();
  }
  scope.postMessage(
    errorCode
      ? { type: "error", code: errorCode, role: affectedRole }
      : { type: "done", file: result },
  );
};
