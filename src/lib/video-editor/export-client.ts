...
export async function exportComposition(
  ...
  if (!compositionSchema.safeParse(composition).success || !composition.main)
    throw new Error("EDITOR_COMPOSITION_INVALID");
  if (
    !globalThis.Worker ||
    ...
    return await new Promise<ExportResult>((resolve, reject) => {
      void navigator.locks
        .request(`${ROOT}:${attempt}`, async () => {
          ...
          let settled = false,
            removing: Promise<void> | null = null,
            ...
            (removing ||= (async () => {
              ...
              try {
                ...
                for (let retry = 0; retry < 3; retry++) {
                  try {
                    await root.removeEntry(attempt, { recursive: true });
                    return;
                  } catch (error) {
                    if (
          ...
          const fail = async (code: string) => {
            ...
            try {
              ...
              /* Next attempt sweeps an orphan after the lock is released. */
            }
            reject(new Error(code));
          };
          const cancel = () => {
            worker.postMessage({ type: "cancel" });
            cancelTimer = setTimeout(() => void fail("EDITOR_CANCELLED"), 1000);
          };
          worker.onerror = () => {
            void fail("EDITOR_EXPORT_FAILED");
          };
          worker.onmessage = ({ data }) => {
            ...
            if (data.type === "progress") options.onProgress(data.progress);
              ...
              if (options.signal.aborted) {
                void fail("EDITOR_CANCELLED");
                return;
              }
              settled = true;
              clearTimeout(cancelTimer);
              options.signal.removeEventListener("abort", cancel);
              resolve({ file: data.file, release: remove });
            } else
              void fail(
                `${data.code || "EDITOR_EXPORT_FAILED"}${data.role ? `:${data.role}` : ""}`,
              );
          };
          options.signal.addEventListener("abort", cancel, { once: true });
          if (options.signal.aborted) {
            await fail("EDITOR_CANCELLED");
            return;
          }
          worker.postMessage({
            type: "export",
