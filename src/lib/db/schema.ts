...
export const referralAttributions = pgTable("referral_attributions", {
  id: text("id").primaryKey(), referralCodeId: text("referral_code_id").notNull(), referrerWorkspaceId: text("referrer_workspace_id").notNull(), referredIdentityDigest: text("referred_identity_digest").notNull(), referredWorkspaceId: text("referred_workspace_id").references(() => workspaces.id, { onDelete: "restrict" }), state: text("state").notNull(), attributionDigest: text("attribution_digest").notNull(), attributedAt: timestamp("attributed_at", { withTimezone: true }).notNull(), qualifiedAt: timestamp("qualified_at", { withTimezone: true }),
}, (table) => ({ codeFk: foreignKey({ name: "referral_attributions_code_fk", columns: [table.referrerWorkspaceId, table.referralCodeId], foreignColumns: [workspaceReferralCodes.workspaceId, workspaceReferralCodes.id] }).onDelete("restrict"), recipientUnique: uniqueIndex("referral_attributions_recipient_unique").on(table.referredIdentityDigest), referrerIdx: index("referral_attributions_referrer_idx").on(table.referrerWorkspaceId, table.state, table.attributedAt), valuesCheck: check("referral_attributions_values_check", sql`${table.state} in ('attributed','qualified','rewarded','fraud_hold','refunded','clawed_back','rejected') and ${table.referredIdentityDigest} ~ '^sha256:[a-f0-9]{64}$' and ${table.attributionDigest} ~ '^sha256:[a-f0-9]{64}$'`) }));
