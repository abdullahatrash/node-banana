...
describe("PersonaTrainingDispatcher", () => {
  it("submits once and persists the provider reference before waiting", async () => {
    const repository = { claimTrainingDispatch: vi.fn().mockResolvedValue(claim), recordProviderTraining: vi.fn(), markTrainingOutcomeUnknown: vi.fn(), resolveTraining: vi.fn() };
    ...
    const dispatcher = new PersonaTrainingDispatcher(repository as unknown as CreatorPersonaRepository, provider, projection, vi.fn().mockResolvedValue({ downloadUrl: "https://signed.invalid/source", key: "key", expiresInSeconds: 3600 }), () => new Date("2026-09-04T12:00:00Z"));
    ...
    expect(repository.recordProviderTraining).toHaveBeenCalledWith(expect.objectContaining({ providerJobRef: "provider-job" }));
  });
  ...
  it("recovers stale submitted effects by stable idempotency key", async () => {
    ...
    const repository = { claimTrainingDispatch: vi.fn().mockResolvedValue(stale), recordProviderTraining: vi.fn(), markTrainingOutcomeUnknown: vi.fn(), resolveTraining: vi.fn().mockResolvedValue({ state: "review" }) };
    const provider = { submit: vi.fn(), recover: vi.fn().mockResolvedValue({ state: "succeeded", providerJobRef: "provider-job", model }) };
    const projection = { synchronize: vi.fn() };
    const dispatcher = new PersonaTrainingDispatcher(repository as unknown as CreatorPersonaRepository, provider, projection, vi.fn(), () => new Date("2026-09-04T12:00:00Z"));
    ...
    expect(repository.resolveTraining).toHaveBeenCalledWith(expect.objectContaining({ resultModelRef: model, idempotencyKey: "provider-training:job:succeeded" }));
  });

  it("holds ambiguous gateway failures for recovery", async () => {
    const repository = { claimTrainingDispatch: vi.fn().mockResolvedValue(claim), recordProviderTraining: vi.fn(), markTrainingOutcomeUnknown: vi.fn(), resolveTraining: vi.fn() };
    const projection = { synchronize: vi.fn() };
    const dispatcher = new PersonaTrainingDispatcher(repository as unknown as CreatorPersonaRepository, { submit: vi.fn().mockRejectedValue(new Error("timeout")), recover: vi.fn() }, projection, vi.fn().mockResolvedValue({ downloadUrl: "https://signed.invalid/source" }), () => new Date("2026-09-04T12:00:00Z"));
    await expect(dispatcher.dispatchOne()).resolves.toEqual({ kind: "outcome_unknown", trainingJobId: "job" });
    expect(repository.markTrainingOutcomeUnknown).toHaveBeenCalledWith(expect.objectContaining({ trainingJobId: "job" }));
  });
});
