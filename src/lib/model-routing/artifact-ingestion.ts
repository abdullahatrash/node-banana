...
export class S3CanonicalArtifactIngestion implements CanonicalArtifactIngestionPort {
  ...
  async ingest(input: Parameters<CanonicalArtifactIngestionPort["ingest"]>[0]) {
    ...
    for (const [outputIndex, value] of urls.entries()) {
      ...
      const assetType = input.intent.outputContract.mediaType; if (assetType === "text") throw new Error("ARTIFACT_MEDIA_CONTRACT_REQUIRED"); const proposedStorageKey = buildAssetObjectKey({ workspaceId: input.workspaceId, projectId: null, assetType, fileExtension: assetType === "video" ? "mp4" : "png" }); const owner = randomUUID(); const at = this.now();
      ...
      try {
        const downloaded = await download(this.fetcher, url, path, declared, this.allowedHosts); await lease.assertOwned(); const decoded = await probe(path, assetType); validateDecodedArtifact(input.intent.outputContract, decoded); const metadata = recordMetadata(input.intent, input.providerPredictionId, outputIndex, downloaded.digest, decoded.durationSeconds, decoded.fps);
