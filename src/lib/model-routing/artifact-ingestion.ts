...
async function download(fetcher: typeof fetch, url: URL, path: string, declared: number): Promise<{ sizeBytes: number; digest: `sha256:${string}` }> {
  const response = await fetcher(url, { cache: "no-store", signal: AbortSignal.timeout(60_000) }); if (!response.ok || !response.body) throw new Error("REPLICATE_OUTPUT_FETCH_FAILED");
...
export class S3CanonicalArtifactIngestion implements CanonicalArtifactIngestionPort {
  ...
  async ingest(input: Parameters<CanonicalArtifactIngestionPort["ingest"]>[0]) {
    ...
    for (const [outputIndex, value] of urls.entries()) {
      const url = new URL(value); if (url.protocol !== "https:" || !this.allowedHosts.some((host) => url.hostname === host || url.hostname.endsWith(`.${host}`))) throw new Error("REPLICATE_OUTPUT_HOST_NOT_ALLOWED");
