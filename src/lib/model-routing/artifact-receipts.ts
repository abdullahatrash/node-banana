...
export interface ArtifactReceiptPort {
  ...
  complete(input: { workspaceId: string; predictionId: string; outputIndex: number; intentId: string; leaseOwner: string; assetId: string; mimeType: string; sizeBytes: number; width: number; height: number; durationSeconds: number | null; fps: number | null; contentDigest: `sha256:${string}`; at: Date }): Promise<ArtifactReceipt>;
}
...
export class PostgresArtifactReceiptRepository implements ArtifactReceiptPort {
  ...
  async claim(input: Parameters<ArtifactReceiptPort["claim"]>[0]) {
    return this.database().transaction(async (tx) => {
      ...
      const [leased] = row.leaseOwner === input.leaseOwner ? [row] : await tx.update(modelArtifactIngestionReceipts).set({ leaseOwner: input.leaseOwner, leaseExpiresAt: input.leaseExpiresAt, updatedAt: input.at }).where(and(eq(modelArtifactIngestionReceipts.workspaceId, input.workspaceId), eq(modelArtifactIngestionReceipts.predictionId, input.predictionId), eq(modelArtifactIngestionReceipts.outputIndex, input.outputIndex), eq(modelArtifactIngestionReceipts.status, "claimed"))).returning();
