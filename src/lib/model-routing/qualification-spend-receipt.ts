...
export function verifyQualificationSpendAuthorization(value: unknown, trustedKeys: Readonly<Record<string, string>>, expected: { account: QualificationProviderAccount; model: string; version: string; capability: QualificationSpendAuthorization["capability"]; billableQuantity: number }): QualificationSpendAuthorization {
...
export function verifyQualificationSpendReceipt(
  ...
  return { ...receipt, credentialFingerprint: receipt.credentialFingerprint as `sha256:${string}`, digest: receipt.digest as `sha256:${string}`, signingKeyId: signature.keyId };
}
