...
export async function getRoutingData() { const [catalog, grants] = await Promise.all([request<{ snapshot: string; items: ModelDescriptor[] }>("/api/studio/model-routing/catalog"), request<{ items: FallbackAuthorization[] }>("/api/studio/model-routing/authorizations")]); return { catalog, grants: grants.items }; }
