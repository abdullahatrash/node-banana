...
function signedQualification(overrides: Record<string, unknown> = {}) {
  ...
  const attestation = {
    ...
    inputContract: { promptKey: "prompt", brandContextKey: "brand_context", aspectRatioKey: "aspect_ratio", quantityKey: null, imageKey: null, imageMode: "single", safety: { parameterKey: "disable_safety_filter", safeValue: false }, lockedParameters: { disable_safety_filter: false, mode: "standard", resolution: "1080p" } },
...
describe("model qualification catalog", () => {
  ...
  it("requires the exact safe value for positive and negative safety keys", () => {
    const unsafeNegative = signedQualification({ inputContract: { promptKey: "prompt", brandContextKey: "brand_context", aspectRatioKey: "aspect_ratio", quantityKey: null, imageKey: null, imageMode: "single", safety: { parameterKey: "disable_safety_filter", safeValue: false }, lockedParameters: { disable_safety_filter: true } } });
    expect(configuredCatalog(unsafeNegative, trustedKeys, at)[0]?.qualification.status).toBe("unqualified");
    const positive = signedQualification({ inputContract: { promptKey: "prompt", brandContextKey: "brand_context", aspectRatioKey: "aspect_ratio", quantityKey: null, imageKey: null, imageMode: "single", safety: { parameterKey: "safety_filter", safeValue: true }, lockedParameters: { safety_filter: true } } });
