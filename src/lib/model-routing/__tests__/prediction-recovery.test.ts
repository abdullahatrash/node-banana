...
describe("Replicate recovery disposition", () => {
  it("keeps async work held while polling and settles only known-cost success", () => {
    ...
    expect(recoveryDisposition({ state: "failed_known", predictionId: "p", code: "FAILED" }).budgetState).toBe("released");
    expect(recoveryDisposition({ state: "aborted_pre_start", predictionId: "p" }).budgetState).toBe("released");
  });
