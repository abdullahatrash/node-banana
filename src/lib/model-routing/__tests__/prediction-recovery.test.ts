...
describe("Replicate recovery disposition", () => {
  ...
  it("does not release ambiguous spend but releases provider-confirmed cancellation", () => {
    expect(recoveryDisposition({ state: "outcome_unknown", predictionId: "p", code: "LOST" }).budgetState).toBe("outcome_unknown");
    expect(recoveryDisposition({ state: "cancelled", predictionId: "p" }).budgetState).toBe("released");
  });
});
