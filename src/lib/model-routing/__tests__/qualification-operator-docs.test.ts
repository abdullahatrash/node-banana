...
describe("Replicate qualification operator contract", () => {
  it("documents the explicit paid flag, no-call default, durable cap, and every trusted prerequisite", () => {
    ...
    for (const value of ["--execute-paid-smoke", "no provider calls", "0130_model_qualification_artifact_reviews", "below USD 0.40", "signed provider-account billing receipt", "qualify:replicate:review --list", "Legacy qualification rows"]) expect(guide).toContain(value)
