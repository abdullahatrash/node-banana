...
describe("Replicate qualification operator contract", () => {
  it("documents the explicit paid flag, no-call default, durable cap, and every trusted prerequisite", () => {
    ...
    expect(script).toMatch(/^import "\.\/_load-env";/)
  ...
  it("keeps the local setup wizard repeatable and preserves the paid boundary", () => {
    ...
    expect(stages).toContain('REGION_TRUST_JSON="$REGION_TRUST" REGION_KEY_ID="$REGION_KEY_ID" node -e')
    expect(stages).toContain('Buffer.from(encoded, "base64").byteLength < 32')
    expect(stages).toContain('APP_DATA_REGION="local-development"')
    expect(stages).toContain('GOVERNANCE_EXPORT_STORAGE_REGION="$S3_REGION"')
    expect(stages).toContain('GOVERNANCE_IMPORT_STORAGE_REGION="$S3_REGION"')
    expect(stages).toContain('GOVERNANCE_IMPORT_PROCESSING_REGION="$APP_DATA_REGION"')
