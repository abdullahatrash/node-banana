...
export const QUALIFIED_TEST_MODELS: readonly ModelDescriptor[] = CURATED_MODELS.map((model, index) => ({
  ...
  qualification: {
    ...
    executionPriceUsd: model.priceUsd.basis === "components" ? model.priceUsd : { basis: model.priceUsd.basis, amount: model.priceUsd.basis === "second" ? 0.05 : model.priceUsd.amount },
    maxQuantity: model.priceUsd.basis === "components" ? 1 : 30,
    ...
    inputContract: { promptKey: "prompt", aspectRatioKey: "aspect_ratio", quantityKey: "duration", imageKey: "image", imageMode: "single" as const, safety: { parameterKey: "disable_safety_checker", safeValue: false }, lockedParameters: { disable_safety_checker: false, draft: false, resolution: "1080p", audio: false, ...(model.priceUsd.basis === "components" ? { output_megapixels: 1 } : {}) } },
    evidence: {
