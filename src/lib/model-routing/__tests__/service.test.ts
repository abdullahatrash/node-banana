...
describe("ModelRoutingService", () => {
  ...
  it("requires explicit authorization and pins brand, locale, rights, model, quote and reservation", async () => {
    ...
    expect(result.kind).toBe("created"); expect(result.intent?.promptDigest).toMatch(/^sha256:/); expect(JSON.stringify(result.intent)).not.toContain("Arabic campaign"); expect(result.intent?.brand.revision).toBe(3); expect(result.intent?.arabicVariety).toBe("gulf"); expect(result.intent?.fundingMode).toBe("byok");
  });
