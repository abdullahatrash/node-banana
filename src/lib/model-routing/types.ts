...
export interface GenerationIntent {
  ...
  contentExecution?: {
    ...
    workflowInputs: { format: string; script: string; prompt: string; speaker: string; scene: string; captionStyle: string; personaId: string | null; mediaSetIds: string[]; themeRevisionRefs: Array<{ themeId: string; revision: number }>; orderedSources: Array<{ assetId: string; type: string; slotKey: string; slotOrdinal: number }>; durationSeconds: number; aspectRatio: "9:16"; contentLanguage: ContentLanguage; arabicVariety: ArabicVariety | null };
