...
export async function admitAndExecuteStudioGeneration(context: { workspaceId: string; userId: string; role: string; planTier: string; idempotencyKey: string; input: AdmittedGenerationInput }): Promise<AdmittedGenerationResult> {
  ...
  const sourceAssetIds = [...new Set(input.sourceAssetIds)];
  const sourceRows = sourceAssetIds.length ? await getDb().select({ id: assets.id, type: assets.type, storageKey: assets.storageKey, checksum: assets.checksum, width: assets.width, height: assets.height, metadata: assets.metadata, createdByUserId: assets.createdByUserId, createdAt: assets.createdAt }).from(assets).where(and(eq(assets.workspaceId, context.workspaceId), inArray(assets.id, sourceAssetIds), isNull(assets.deletedAt))) : [];
  const sourceValidation = validateGenerationSources(input.capability, sourceAssetIds, sourceRows.map((row) => ({ ...row, metadata: row.metadata && typeof row.metadata === "object" && !Array.isArray(row.metadata) ? row.metadata : null })));
  ...
  const executed = await productionGenerationExecution(credential).execute({ workspaceId: context.workspaceId, userId: context.userId, intentId: created.intent.id, rawPrompt: input.prompt, sourceUrls, idempotencyKey: `${context.idempotencyKey}:execute` });
