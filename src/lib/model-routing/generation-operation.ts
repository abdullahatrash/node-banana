...
export function generationOperationMetadata(intent: GenerationIntent) {
  return { provider: intent.selectedModel.provider, intentId: intent.id, model: intent.selectedModel.model, version: intent.selectedModel.version, inputSchemaDigest: intent.selectedModel.inputSchemaDigest, personaId: intent.persona?.personaId ?? null, personaRevision: intent.persona?.personaRevision ?? null, personaEvidenceRefs: intent.persona ? Object.values(intent.persona.evidence).filter((value): value is string => typeof value === "string") : [], brandProfileId: intent.brand.profileId, brandRevision: intent.brand.revision, brandContextDigest: intent.brand.context.digest, brandReferenceAssetIds: intent.brand.context.referenceAssets.map((item) => item.assetId), contentLanguage: intent.contentLanguage, arabicVariety: intent.arabicVariety, fundingMode: intent.fundingMode, quoteAmountUsd: intent.quote.amount, quoteQuantity: intent.quote.quantity, quoteBasis: intent.quote.basis, reservationIds: intent.reservationIds, rightsSnapshotId: intent.rights.snapshotId, rightsSnapshotRevision: intent.rights.revision, rightsEvidenceRefs: intent.rights.evidence.map((item) => item.id), provenanceRefs: intent.rights.sourceAssetIds, region: intent.regionAdmission.region, regionPolicyId: intent.regionAdmission.policyId, regionPolicyVersion: intent.regionAdmission.policyVersion, regionEvidenceDigest: intent.regionAdmission.evidenceDigest, aspectRatio: intent.outputContract.aspectRatio, width: intent.outputContract.width, height: intent.outputContract.height, fps: intent.outputContract.fps, providerState: "admitted", nextAction: "submit_provider" };
}
