...
export class ReplicateHttpClient implements ReplicateClientPort {
  ...
  create(input: { endpoint: "versioned"; model: string; version: string; input: Record<string, unknown>; cancelAfterSeconds: number }) {
    return this.call("/predictions", { method: "POST", headers: { "Cancel-After": `${input.cancelAfterSeconds}s` }, body: JSON.stringify({ version: input.version, input: input.input }) });
  }
