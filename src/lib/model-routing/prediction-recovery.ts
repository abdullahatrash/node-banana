...
import { modelGenerationBudgetReservations, modelProviderEffectClaims } from "./db-schema";
...
export async function recoverReplicatePredictions(input: { database: Db; operations: OperationStatusService; adapterFor(workspaceId: string): Promise<ReplicatePredictionAdapter>; now?: () => Date; limit?: number }) {
  ...
  const claimed = await input.database.transaction(async (tx) => {
    const rows = await tx.select().from(modelProviderEffectClaims).where(and(inArray(modelProviderEffectClaims.state, ["submitted", "outcome_unknown"]), lte(modelProviderEffectClaims.nextPollAt, at), or(isNull(modelProviderEffectClaims.leaseExpiresAt), lte(modelProviderEffectClaims.leaseExpiresAt, at)))).orderBy(asc(modelProviderEffectClaims.nextPollAt)).limit(input.limit ?? 20).for("update", { skipLocked: true });
  ...
  for (const row of claimed) {
    try {
      if (!row.predictionId) { summary.unknown++; await settle(input.database, row.workspaceId, row.intentId, "outcome_unknown", at); continue; }
      const intent = await routing.getIntent(row.workspaceId, row.intentId); if (!intent) { summary.failed++; continue; }
      const result = await (await input.adapterFor(row.workspaceId)).poll(intent, row.predictionId);
      ...
      if (result.state === "waiting_provider") {
        ...
        await settle(input.database, row.workspaceId, row.intentId, terminal, at);
      }
...
export function recoveryDisposition(result: ReplicateExecutionResult): { operationState: OperationState; effectState: "submitted" | "outcome_unknown" | "succeeded" | "failed_known" | "cancelled"; budgetState: "held" | "settled" | "outcome_unknown" } {
  ...
  if (result.state === "failed_known") return { operationState: "failed_known", effectState: "failed_known", budgetState: "settled" };
...
async function settle(database: Db, workspaceId: string, intentId: string, state: OperationState, at: Date) {
  const status = state === "succeeded" || state === "failed_known" ? "settled" : state === "cancelled" || state === "outcome_unknown" ? "outcome_unknown" : "held";
  await database.update(modelGenerationBudgetReservations).set({ status, updatedAt: at }).where(and(eq(modelGenerationBudgetReservations.workspaceId, workspaceId), eq(modelGenerationBudgetReservations.intentId, intentId)));
}
