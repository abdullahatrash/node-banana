...
export const modelQualificationRuns = pgTable("model_qualification_runs", {
  id: text("id").primaryKey(), requestDigest: text("request_digest").notNull(), provider: text("provider").notNull(), model: text("model").notNull(), modelVersion: text("model_version").notNull(), signingKeyId: text("signing_key_id").notNull(), state: text("state").notNull(), hardCapUsd: numeric("hard_cap_usd", { precision: 8, scale: 6 }).notNull(), reservedSpendUsd: numeric("reserved_spend_usd", { precision: 8, scale: 6 }).notNull(), result: jsonb("result").$type<Record<string, unknown>>(), createdAt: timestamp("created_at", { withTimezone: true }).notNull(), updatedAt: timestamp("updated_at", { withTimezone: true }).notNull(), completedAt: timestamp("completed_at", { withTimezone: true }),
}, (table) => ({
  valuesCheck: check("model_qualification_runs_values_check", sql`${table.requestDigest} ~ '^sha256:[a-f0-9]{64}$' and ${table.provider} = 'replicate' and ${table.state} in ('running','completed','blocked') and ${table.hardCapUsd}::numeric = 0.4 and ${table.reservedSpendUsd}::numeric > 0 and ${table.reservedSpendUsd}::numeric < ${table.hardCapUsd}::numeric`),
...
export const modelQualificationCases = pgTable("model_qualification_cases", {
  ...
  valuesCheck: check("model_qualification_cases_values_check", sql`${table.requestDigest} ~ '^sha256:[a-f0-9]{64}$' and ${table.state} in ('reserved','submitting','submitted','outcome_unknown','completed') and ${table.maximumSpendUsd}::numeric > 0 and ${table.maximumSpendUsd}::numeric < 0.4 and length(${table.submissionKey}) between 16 and 300 and (${table.state} <> 'completed' or (${table.predictionId} is not null and ${table.terminalStatus} in ('succeeded','failed','canceled','aborted') and ${table.result} is not null and ${table.completedAt} is not null))`),
