...
export class GenerationOperationControlAdapter implements OperationControlAdapter {
  ...
  constructor(private readonly database: () => Db, private readonly settle = settleGenerationSpend) {}
  async cancel(operation: Parameters<OperationControlAdapter["cancel"]>[0]) {
    ...
    if (operation.state === "queued" || operation.state === "admitted") {
      await this.settle({ database: this.database(), workspaceId: operation.workspaceId, intentId: operation.resourceId, outcome: { kind: "pre_start_cancelled" }, quotedAmountUsd: quote, at: new Date() });
