...
export function assertContentModelPolicy(input: {
  definition: ContentFormatDefinition; intent: GenerationIntent; descriptor: ModelDescriptor;
}): void {
  const { definition, intent, descriptor } = input;
  const policy = definition.execution.modelPolicy;
  if (!policy || policy.id !== `content.${definition.format}.v2` || policy.revision !== 2 || !policy.qualifiedModelsOnly) throw new ContentWorkflowRuntimeError("CONTENT_MODEL_POLICY_UNAVAILABLE");
...
export class ContentGenerationWorkflowService {
  ...
  async start(input: {
    ...
    if (binding.workflow.id !== input.definition.execution.workflow.id || binding.workflow.revisionId !== input.definition.execution.workflow.revisionId || binding.formatDefinition.id !== input.definition.id || binding.formatDefinition.revision !== input.definition.revision) throw new ContentWorkflowRuntimeError("CONTENT_WORKFLOW_REVISION_MISMATCH");
    assertContentModelPolicy({ definition: input.definition, intent: input.intent, descriptor: input.descriptor });
    ...
    const accepted = await this.runtime.start({
      ...
      inputs: { request }, idempotencyKey: `${input.idempotencyKey}:workflow`,
