...
export function validatePersistedContentFormatDefinition(value: unknown): ContentFormatDefinition {
  ...
  if (!builtin || candidate.id !== `content-format:${candidate.format}` || !candidate.layout || !sameStringSet(candidate.layout.aspectRatios, ["9:16"]) || candidate.layout.defaultAspectRatio !== "9:16" || !candidate.execution || !candidate.renderProof || !["content-render-proof/v1", "content-render-proof/v2"].includes(candidate.renderProof.schema) || !sameStringSet(candidate.renderProof.verifies, ["fonts", "bidi", "captions", "timing", "safe_areas"]) || !candidate.editorHandoff?.requiresPassedRenderProof || !Array.isArray(candidate.controls) || !Array.isArray(candidate.requiredControls) || !Array.isArray(candidate.sourceSlots)) throw new ContentFormatRegistryError("CONTENT_FORMAT_DEFINITION_INVALID");
