...
export const CONTENT_WORKFLOW_OPERATION_REGISTRY_DIGEST = canonicalDigest({
  ...
  contentDispatch: CONTENT_GENERATION_DISPATCH_OPERATION.contractDigest,
});
...
export function resolvedContentWorkflowDefinition(definition: ContentFormatDefinition): ResolvedWorkflowDefinition {
  ...
  return {
    ...
    inputs: { request: { kind: "text", required: true } }, credentialSlots: {},
    steps: [
      { id: "guard", operation: { identity: digestOperation.identity, contractDigest: digestOperation.contractDigest }, inputs: { text: { from: "workflow_input", input: "request" } }, credentials: {}, config: {}, retry: { maxAttempts: 1, backoff: { initialMs: 0, maxMs: 0, multiplier: 1 } } },
      { id: "dispatch", operation: { identity: CONTENT_GENERATION_DISPATCH_OPERATION.identity, contractDigest: CONTENT_GENERATION_DISPATCH_OPERATION.contractDigest }, inputs: { request: { from: "workflow_input", input: "request" }, guard: { from: "step_output", step: "guard", output: "textDigest" } }, credentials: {}, config: {}, retry: { maxAttempts: 1, backoff: { initialMs: 0, maxMs: 0, multiplier: 1 } } },
    ],
    outputs: { receipt: { kind: "text", binding: { from: "step_output", step: "dispatch", output: "receipt" } } },
  };
}
