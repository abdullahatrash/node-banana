...
function fixture(format: Exclude<(typeof CONTENT_FORMATS)[number], "custom_upload">) {
  ...
  const workflowInputs = { format, script: `script:${format}`, prompt: `prompt:${format}`, speaker: `speaker:${format}`, scene: `scene:${format}`, captionStyle: "brand", personaId: definition.controls.includes("persona") ? "persona_1" : null, mediaSetRevisions: [], themeInstructions: [], orderedSources: sourceAssetIds.map((assetId, index) => ({ assetId, type: definition.sourceSlots[index]?.type ?? definition.sourceSlots[0]?.type ?? "image", slotKey: definition.sourceSlots[index]?.key ?? definition.sourceSlots[0]?.key ?? "images", slotOrdinal: index })), durationSeconds: 5, aspectRatio: "9:16" as const, contentLanguage: "ar" as const, arabicVariety: "gulf" as const };
  const policyUnsigned = { schema: "content-model-policy/v1" as const, id: definition.execution.modelPolicy!.id, revision: definition.execution.modelPolicy!.revision, format, region: "replicate-us" as const, defaultModel: modelRef, compatibleModels: [modelRef], overrides: { mode: "explicit_exact_allowlist" as const, allowedFields: ["model"] as const, requireRequote: true as const } };
  const modelPolicy = { id: policyUnsigned.id, revision: policyUnsigned.revision, qualifiedModelsOnly: true as const, digest: canonicalDigest(policyUnsigned), region: "replicate-us", defaultModel: modelRef, compatibleModels: [modelRef], overrideMode: "explicit_exact_allowlist" as const };
  const unsigned = {
