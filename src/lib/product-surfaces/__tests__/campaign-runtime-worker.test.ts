...
describe("Product campaign runtime worker", () => {
  it("materializes a bounded horizon, runs daily replenishment, then dispatches due work", async () => {
    const scheduleStore = { schedule: vi.fn().mockResolvedValue({ inserted: 3, replayed: 1 }), markStaleSubmissionsUnknown: vi.fn().mockResolvedValue(1) };
    ...
    await expect(worker.run({ workerId: "worker" })).resolves.toMatchObject({ scanned: 1, scheduled: 3, scheduleReplayed: 1, replenished: 2, staleUnknown: 1, occurrences: { started: 1 } });
  ...
  it("isolates an invalid campaign without blocking due occurrence recovery", async () => {
    const scheduleStore = { schedule: vi.fn(), markStaleSubmissionsUnknown: vi.fn().mockResolvedValue(0) };
