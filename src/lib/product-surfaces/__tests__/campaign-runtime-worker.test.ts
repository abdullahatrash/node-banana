...
describe("Product campaign runtime worker", () => {
  it("materializes a bounded horizon, runs daily replenishment, then dispatches due work", async () => {
    ...
    const replenisher = { replenish: vi.fn().mockResolvedValue({ kind: "completed", created: 2, replayed: 0, stopReason: "run_limit_reached" }), recoverExpired: vi.fn().mockResolvedValue({ found: 1, requeued: 1, failed: 0 }) };
    ...
    await expect(worker.run({ workerId: "worker" })).resolves.toMatchObject({ scanned: 1, scheduled: 3, scheduleReplayed: 1, replenished: 2, replenishmentRecovery: { found: 1, requeued: 1, failed: 0 }, staleUnknown: 1, reconciled: 2, occurrences: { started: 1 } });
  ...
  it("isolates an invalid campaign without blocking due occurrence recovery", async () => {
    ...
    const worker = new ProductCampaignRuntimeWorker(scheduleStore as never, scheduler as never, { replenish: vi.fn(), recoverExpired: vi.fn().mockResolvedValue({ found: 0, requeued: 0, failed: 0 }) } as never, () => now, campaignPage as never);
