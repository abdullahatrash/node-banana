...
describe("Content Model Policy", () => {
  it("persists an exact signed-qualified default and explicit compatible allowlist", () => {
    ...
    expect(policy).toMatchObject({ id: "content.slideshow.v3", revision: 3, region: "replicate-us", defaultModel: { model: "prunaai/p-video", version: "pinned-v1" }, overrides: { mode: "explicit_exact_allowlist", requireRequote: true } });
