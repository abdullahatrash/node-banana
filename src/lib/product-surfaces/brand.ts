...
import { brandAnalysisRuns, brandProfiles, brandSources, onboardingAnalysisDispatchIntents, onboardingCommandReceipts } from "@/lib/db/schema";
import { brandProfileCorrectionSchema, brandProfileV1Schema, type BrandProfileCorrection } from "@/lib/onboarding/schemas";
import { brandCommandRequestDigest, brandReceiptRevision, draftFollowsActiveBrandLineage } from "./brand-command-policy";
...
export async function refreshBrandSource(input: { workspaceId: string; userId: string; sourceId: string; expectedSourceRevision: number; idempotencyKey: string; now?: Date }) {
  const digest = brandCommandRequestDigest({ workspaceId: input.workspaceId, action: "refresh_source", facts: { sourceId: input.sourceId, expectedSourceRevision: input.expectedSourceRevision } });
  ...
  return getDb().transaction(async (tx) => {
    ...
    await tx.insert(onboardingCommandReceipts).values({ userId: input.userId, idempotencyKey: input.idempotencyKey, commandType: "brand_source.refresh", requestFingerprint: digest, sessionRevision: brandReceiptRevision(sourceRevision), result, createdAt: now });
    return result;
  });
}
...
export async function createBrandRevision(input: { workspaceId: string; userId: string; expectedActiveRevision: number; correction: BrandProfileCorrection; idempotencyKey: string; now?: Date }) {
  ...
  const digest = brandCommandRequestDigest({ workspaceId: input.workspaceId, action: "create_revision", facts: { expectedActiveRevision: input.expectedActiveRevision, correction } });
  const now = input.now ?? new Date();
  return getDb().transaction(async (tx) => {
    const [receipt] = await tx.select().from(onboardingCommandReceipts).where(and(eq(onboardingCommandReceipts.userId, input.userId), eq(onboardingCommandReceipts.idempotencyKey, input.idempotencyKey))).limit(1);
    if (receipt) {
    ...
    if (!active) throw new Error("Active Brand Profile is unavailable.");
    ...
    await tx.insert(onboardingCommandReceipts).values({ userId: input.userId, idempotencyKey: input.idempotencyKey, commandType: "brand_profile.create_revision", requestFingerprint: digest, sessionRevision: brandReceiptRevision(revision), result, createdAt: now });
    return result;
  });
}
...
export async function activateBrandRevision(input: { workspaceId: string; userId: string; profileId: string; expectedRevision: number; idempotencyKey: string; now?: Date }) {
  const digest = brandCommandRequestDigest({ workspaceId: input.workspaceId, action: "activate_revision", facts: { profileId: input.profileId, expectedRevision: input.expectedRevision } });
  const now = input.now ?? new Date();
  return getDb().transaction(async (tx) => {
    const [receipt] = await tx.select().from(onboardingCommandReceipts).where(and(eq(onboardingCommandReceipts.userId, input.userId), eq(onboardingCommandReceipts.idempotencyKey, input.idempotencyKey))).limit(1);
    if (receipt) {
    ...
    const [[active], [draft]] = await Promise.all([
      tx.select({ id: brandProfiles.id }).from(brandProfiles).where(and(eq(brandProfiles.workspaceId, input.workspaceId), eq(brandProfiles.status, "active"))).limit(1),
    ...
    if (!active || !draft || !draftFollowsActiveBrandLineage({ draftSourceProfileId: draft.sourceProfileId, activeProfileId: active.id })) throw new BrandRevisionConflictError("Draft Brand Profile is no longer current.");
    ...
    await tx.insert(onboardingCommandReceipts).values({ userId: input.userId, idempotencyKey: input.idempotencyKey, commandType: "brand_profile.activate_revision", requestFingerprint: digest, sessionRevision: brandReceiptRevision(activated.revision), result: activated, createdAt: now });
    return activated;
  });
}
