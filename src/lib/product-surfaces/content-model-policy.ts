...
export function resolveContentModelPolicy(definition: ContentFormatDefinition, catalog: readonly ModelDescriptor[]): ContentModelPolicy | null {
  if (definition.format === "custom_upload" || !definition.execution.modelPolicy || definition.execution.modelPolicy.revision !== definition.revision || definition.execution.modelPolicy.id !== `content.${definition.format}.v${definition.revision}` || !definition.execution.capability) return null;
  ...
  const unsigned = { schema: "content-model-policy/v1" as const, id: definition.execution.modelPolicy.id, revision: definition.execution.modelPolicy.revision, format: definition.format, region: "replicate-us" as const, defaultModel: allowed[0], compatibleModels: allowed, overrides: { mode: "explicit_exact_allowlist" as const, allowedFields: ["model"] as const, requireRequote: true as const } };
