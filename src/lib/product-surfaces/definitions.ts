...
const trendEvidenceSchema = z.object({
  ...
  source: z.object({
    ...
    observationProvenance: z.object({ kind: z.enum(["workspace_attested", "platform_verified"]), ref: text(500), digest: sha256Digest }).strict().nullable().default(null),
  }).strict(),
...
export const inspirationPayloadSchema = z.object({
  ...
  metrics: z.object({ views: z.number().int().nonnegative().nullable(), likes: z.number().int().nonnegative().nullable(), comments: z.number().int().nonnegative().nullable().optional() }).refine((metrics) => Object.values(metrics).some((value) => value !== null)),
