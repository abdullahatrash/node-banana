...
export function validateBrandAwareBlitzGenerationContract(input: {
  ...
  if (!briefResult.success || !payload.inspirationItemId || !payload.sourceAssetId || !payload.sourceMediaType || !payload.rightsSnapshot || !payload.rightsBasis || !payload.permittedRemix || !payload.contentLanguage) return { ok: false, code: "BLITZ_BRIEF_SNAPSHOT_REQUIRED" };
  const brief = briefResult.data;
  const exact = input.request.prompt === brief.provider.prompt
