...
import { buildBlitzSimilarityGate, validateBlitzSimilarityGate, type BlitzSimilarityGateEvidence } from "./blitz-similarity-policy";
...
export async function evaluateAndStoreBlitzSimilarity(input: { workspaceId: string; itemId: string; expectedRevision: number; candidateAssetId: string }, dependencies: Dependencies = productionDependencies()) {
  ...
  if (existing) {
    ...
    const validation = validateBlitzSimilarityGate({ evidence, sourceAsset: { id: source.id, contentDigest: source.checksum as `sha256:${string}` }, candidateAsset: { id: candidate.id, contentDigest: candidate.checksum as `sha256:${string}` } });
  ...
  const evaluated = await dependencies.evaluator.evaluate({ source: { assetId: source.id, contentDigest: source.checksum as `sha256:${string}`, downloadUrl: sourceUrl }, candidate: { assetId: candidate.id, contentDigest: candidate.checksum as `sha256:${string}`, downloadUrl: candidateUrl } });
  const evidence = buildBlitzSimilarityGate({ sourceAsset: { id: source.id, contentDigest: source.checksum as `sha256:${string}` }, candidateAsset: { id: candidate.id, contentDigest: candidate.checksum as `sha256:${string}` }, measurements: evaluated.measurements, evaluatedAt: dependencies.now(), evaluator: { kind: "qualified_internal", adapterId: evaluated.evaluator.id, adapterVersion: evaluated.evaluator.version, qualificationDigest: evaluated.evaluator.qualificationDigest } });
  return db.transaction(async (tx) => {
    ...
    const validation = validateBlitzSimilarityGate({ evidence, sourceAsset: { id: currentSource.id, contentDigest: currentSource.checksum as `sha256:${string}` }, candidateAsset: { id: currentCandidate.id, contentDigest: currentCandidate.checksum as `sha256:${string}` } });
    if (!validation.ok && validation.code !== "BLITZ_SIMILARITY_BLOCKED") throw new BlitzSimilarityServiceError(validation.code);
...
export async function requirePassedBlitzSimilarityEvidence(executor: Parameters<Parameters<ReturnType<typeof getDb>["transaction"]>[0]>[0], input: { workspaceId: string; evidenceId: string; itemId: string; itemRevision: number; sourceAssetId: string; sourceDigest: string; candidateAssetId: string; candidateDigest: string }) {
  ...
  const validation = validateBlitzSimilarityGate({ evidence: row?.evidence as unknown as BlitzSimilarityGateEvidence ?? null, sourceAsset: { id: input.sourceAssetId, contentDigest: input.sourceDigest as `sha256:${string}` }, candidateAsset: { id: input.candidateAssetId, contentDigest: input.candidateDigest as `sha256:${string}` } });
