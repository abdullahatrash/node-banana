...
export function contentProviderPrompt(payloadValue: unknown): string {
  const payload = contentPieceSchema.parse(payloadValue);
  return contentProviderPromptFromWorkflowInputs({ format: payload.format, script: payload.script ?? "", prompt: payload.prompt ?? "", speaker: payload.speaker ?? "", scene: payload.scene ?? "", captionStyle: payload.captionStyle ?? "", personaId: payload.personaId, mediaSetIds: payload.mediaSetIds, themeRevisionRefs: payload.themeRevisionRefs, orderedSources: payload.sourceAssetIds.map((assetId) => ({ assetId })), durationSeconds: payload.durationSeconds, aspectRatio: "9:16", contentLanguage: payload.contentLanguage, arabicVariety: payload.arabicVariety });
}

export function contentProviderPromptFromWorkflowInputs(input: Pick<NonNullable<GenerationIntent["contentExecution"]>["workflowInputs"], "format" | "script" | "prompt" | "speaker" | "scene" | "captionStyle" | "personaId" | "mediaSetIds" | "themeRevisionRefs" | "durationSeconds" | "aspectRatio" | "contentLanguage" | "arabicVariety"> & { orderedSources: Array<{ assetId: string }> }): string {
  return canonicalJson({ schema: "content-provider-controls/v1", format: input.format, script: input.script, prompt: input.prompt, speaker: input.speaker, scene: input.scene, captionStyle: input.captionStyle, personaId: input.personaId, mediaSetIds: input.mediaSetIds, themeRevisionRefs: input.themeRevisionRefs, orderedSourceAssetIds: input.orderedSources.map((source) => source.assetId), durationSeconds: input.durationSeconds, aspectRatio: input.aspectRatio, contentLanguage: input.contentLanguage, arabicVariety: input.arabicVariety });
}
