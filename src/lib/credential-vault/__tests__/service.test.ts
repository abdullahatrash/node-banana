...
async function fixture(
  mode: "bounded" | "audited_unbounded" = "bounded",
...
describe("Credential Vault", () => {
  ...
  it("keeps generated bytes transient while persisting only a safe receipt summary", async () => {
    const value = await fixture();
    const intent = await snapshot(value, "transient-effect");
    const invoke = vi.fn(async (credential: { secret: string }) => ({
      bytes: new Uint8Array([137, 80, 78, 71]),
      providerOperationRef: "gemini-response-1",
      credentialWasPresent: credential.secret === "sk-private-value-1234",
    }));
    const first = await value.executor.withCredentialMaterialForEffect({
      securityContext,
      effectIntent: intent,
      providerIntent: { prompt: "hello" },
      invoke,
    ...
    expect(first).toMatchObject({
      replayed: false,
      result: { credentialWasPresent: true },
    });
    expect(JSON.stringify(first)).not.toContain("sk-private");
    const replay = await value.executor.withCredentialMaterialForEffect({
      securityContext,
      effectIntent: intent,
      providerIntent: { prompt: "hello" },
      invoke,
      summarize: () => ({ outcome: "should-not-run" }),
    });
    ...
    expect(JSON.stringify(value.repository.spendEvents)).not.toContain(
      "137",
    );
  });

  it("rejects unsafe callback summaries and never persists the secret", async () => {
    const value = await fixture();
    const intent = await snapshot(value, "unsafe-summary-effect");
    await expect(
      value.executor.withCredentialMaterialForEffect({
        securityContext,
        effectIntent: intent,
        providerIntent: { prompt: "hello" },
        invoke: async (credential) => ({ secret: credential.secret }),
        summarize: (result) => ({ secret: result.secret }),
      }),
  ...
  it.each(["base64", "base64url", "hex"] as const)(
    "rejects %s-encoded Credential material in transient provider output",
    async (encoding) => {
      const value = await fixture();
      const intent = await snapshot(value, `unsafe-${encoding}-effect`);
      await expect(
        value.executor.withCredentialMaterialForEffect({
          securityContext,
          effectIntent: intent,
          ...
          invoke: async (credential) => ({
            output: Buffer.from(credential.secret, "utf8").toString(encoding),
          }),
          summarize: () => ({ outcome: "succeeded" }),
        }),
      ).rejects.toMatchObject({
        outcome: "unknown",
        failureCode: "UNSAFE_PROVIDER_RESULT",
  ...
  ])("rejects $name-encoded Credential material", async (testCase) => {
    const value = await fixture("bounded", testCase.secret);
    const intent = await snapshot(value, `unsafe-${testCase.name}-effect`);
    await expect(
      value.executor.withCredentialMaterialForEffect({
        securityContext,
        effectIntent: intent,
        providerIntent: { prompt: "hello" },
        invoke: async (credential) => ({
          output: testCase.encode(credential.secret),
        }),
        summarize: () => ({ outcome: "succeeded" }),
      }),
    ).rejects.toMatchObject({
  ...
  it.each([
    {
      effectRef: "callback-not-created",
      disposition: {
  ...
  ])("settles $effectRef with its truthful effect disposition", async (testCase) => {
    const value = await fixture();
    const intent = await snapshot(value, testCase.effectRef);
    const result = await value.executor.withCredentialMaterialForEffect({
      securityContext,
      effectIntent: intent,
      providerIntent: { prompt: "hello" },
      invoke: async () => ({ kind: testCase.disposition.kind }),
      summarize: (outcome) => ({ kind: outcome.kind }),
      disposition: () => testCase.disposition,
    });
    expect(result.replayed).toBe(false);
    expect(
      value.repository.spendEvents.find(
        (event) => event.effectRef === testCase.effectRef,
      )?.status,
    ).toBe(testCase.status);
    if (testCase.status === "failed") {
      await expect(
        value.executor.inspectEffectForRecovery({
          securityContext,
          effectRef: testCase.effectRef,
        }),
      ).resolves.toMatchObject({
        kind: "failed",
        safeResult: { kind: "failed_not_started" },
      });
    }
  });
