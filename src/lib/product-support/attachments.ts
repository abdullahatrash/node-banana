...
export async function resolveSupportAttachmentReferences(input: { workspaceId: string; assetIds: string[]; capturedAt?: Date }) {
  ...
  const rows = await getDb().select({ id: assets.id, type: assets.type, mimeType: assets.mimeType, sizeBytes: assets.sizeBytes, checksum: assets.checksum, metadata: assets.metadata }).from(assets).where(and(eq(assets.workspaceId, input.workspaceId), inArray(assets.id, ids), isNull(assets.deletedAt)));
