...
describe("ConfiguredMerchantOfRecordAdapter", () => {
  ...
  it("accepts only a fresh signature over the exact webhook bytes", () => {
    ...
    expect(adapter.verifyWebhook({ body, timestamp, signature, paddleSignature: null, at: new Date("2026-09-01T00:02:00.000Z") })).toMatchObject({ kind: "checkout_event", event: { eventId: "evt_1" } });
