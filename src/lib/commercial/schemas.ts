...
export const internalCommercialCommandSchema = z.discriminatedUnion("action", [
  ...
  z.object({ action: z.literal("issue_quote"), workspaceId: id, purposeRef: id, maxCreditDebit: z.number().int().positive(), pricingSnapshotDigest: digest, expiresAt: date, localPriceMinor: z.number().int().nonnegative().nullable(), currency: z.string().regex(/^[A-Z]{3}$/).nullable(), taxMinor: z.number().int().nonnegative().nullable() }),
  z.object({ action: z.literal("reserve_quote"), workspaceId: id, quoteId: id, externalEffectRef: id.nullable() }),
  z.object({ action: z.literal("settle_reservation"), workspaceId: id, reservationId: id, outcome: z.enum(["succeeded", "failed_known", "outcome_unknown"]), actualDebitUnits: z.number().int().nonnegative().nullable() }),
  z.object({ action: z.literal("grant_purchased_credits"), workspaceId: id, merchantReceiptRef: id, units: z.number().int().positive() }),
  z.object({ action: z.literal("attribute_referral"), code: z.string().regex(/^[A-Z0-9-]{6,32}$/), referredIdentityDigest: digest, referredWorkspaceId: id.nullable(), attributionDigest: digest }),
  ...
  z.object({ action: z.literal("transition_subscription"), workspaceId: id, expectedRevision: z.number().int().positive(), toState: z.enum(["active", "past_due", "grace", "cancel_at_period_end", "cancelled", "suspended"]), reasonCode: z.string().regex(/^[a-z][a-z0-9_.-]{2,99}$/), periodEndsAt: date, graceEndsAt: date.nullable(), merchantCustomerRef: id.nullable(), merchantSubscriptionRef: id.nullable() }),
]);
