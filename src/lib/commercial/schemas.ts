...
export const internalCommercialCommandSchema = z.discriminatedUnion("action", [
  ...
  z.object({ action: z.literal("settle_reservation"), workspaceId: id, reservationId: id, outcome: z.enum(["succeeded", "pre_start_cancelled", "outcome_unknown"]), actualDebitUnits: z.number().int().nonnegative().nullable(), idempotencyKey: id }),
