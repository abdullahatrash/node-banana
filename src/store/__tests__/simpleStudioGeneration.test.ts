...
describe("Simple Studio admitted media generation", () => {
  beforeEach(() => { vi.clearAllMocks(); useSimpleStudioStore.setState({ mode: "video", prompt: "إعلان خليجي", rewrittenPrompt: null, rewriteEnabled: false, selectedModelId: "google/veo-3.1-lite", selectedModelProvider: "replicate", selectedModelName: "Veo", selectedModelVersion: "immutable-version-1", selectedModelSchemaDigest: `sha256:${"a".repeat(64)}`, aspectRatio: "9:16", batchCount: 1, sourceImage: null, sourceMediaType: null, videoDuration: 5, dialogueEnabled: false, arabicVariety: "gulf", rightsBasis: "owned", permittedRemix: "transform", rightsConfirmed: true, isGenerating: false, generationsByMode: { photo: [], video: [], copy: [] }, generations: [] }); });
  ...
  it("selects video-to-video admission for an ordered canonical trending video source", async () => {
    ...
    const fetcher = vi.fn(async (_url: RequestInfo | URL, _init?: RequestInit) => ({ ok: true, json: async () => ({ success: true, operation: { id: "operation-2", state: "succeeded", revision: 4, metadata: { artifactIds: ["asset-1"] } } }) } as Response));
