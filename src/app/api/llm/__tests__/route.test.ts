...
describe("/api/llm route", () => {
  ...
  it.each(["", "ws-spoofed"])("fails closed before LLM dispatch when exact Workspace authority is missing (%s)", async (workspaceId) => {
    process.env.GEMINI_API_KEY = "test-gemini-key";
    const response = await POST(createMockPostRequest({ prompt: "test", provider: "google", model: "gemini-2.5-flash" }, { "x-workspace-id": workspaceId }));
