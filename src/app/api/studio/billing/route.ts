...
export const POST = withStudioAuth<undefined>({ route: "/api/studio/billing", action: "write", permission: "product:billing:manage" }, async (request, authz) => {
  ...
  try {
    if (parsed.data.action === "start_trial") {
      const result = await COMMERCIAL.startTrial({ workspaceId: authz.workspaceId, userId: authz.userId, planId: parsed.data.planId, planVersion: parsed.data.planVersion, idempotencyKey: parsed.data.idempotencyKey });
      const source = result as { subscriptionEventId?: unknown; occurredAt?: unknown };
      if (typeof source.subscriptionEventId === "string" && typeof source.occurredAt === "string") await recordMarketingAttributionBestEffort({ workspaceId: authz.workspaceId, userId: authz.userId, email: authz.contentSession.user.email, eventName: "trial_started", occurredAt: new Date(source.occurredAt), idempotencyKey: `xads:trial:${source.subscriptionEventId}` });
