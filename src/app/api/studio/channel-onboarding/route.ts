...
export const POST = withStudioAuth<undefined>({ route: "/api/studio/channel-onboarding", action: "write", permission: "social:manage" }, async (request: NextRequest, authz) => {
  ...
  try {
    if (command.action === "create_order") {
      await requireWorkspaceCommercialFeature(authz.workspaceId, "managedChannelOnboarding");
      return NextResponse.json({ success: true, result: await CHANNEL_ONBOARDING.createOrder({ ...command, workspaceId: authz.workspaceId, userId: authz.userId }) }, { status: 201 });
    }
