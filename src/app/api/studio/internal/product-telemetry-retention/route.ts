...
function authorized(request: NextRequest): boolean { const expected = process.env.RELEASE_TELEMETRY_RETENTION_SECRET; const actual = request.headers.get("authorization")?.replace(/^Bearer\s+/i, ""); if (!expected || expected.length < 32 || !actual) return false; const a = Buffer.from(expected); const b = Buffer.from(actual); return a.length === b.length && timingSafeEqual(a, b); }
...
export async function POST(request: NextRequest) {
  ...
  try { const deleted = await getReleaseControlService().deleteExpiredTelemetry(new Date(), limit); return noStoreJson({ success: true, deleted }); }
