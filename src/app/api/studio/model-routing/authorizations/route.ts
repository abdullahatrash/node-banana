...
const body = z.object({ source: ref, targets: z.array(ref).min(1).max(10), capability: z.enum(["text_generation","text_to_image","image_to_image","text_to_video","image_to_video","video_to_video"]), minimumQuality: z.enum(["preview","standard","premium"]), contentLanguage: z.enum(["ar","en","mixed"]), arabicVariety: z.enum(["msa","gulf","egyptian","levantine","maghrebi","other"]).nullable(), verifiedRegion: z.string().min(1).max(80), executionMode: z.enum(["sync","async"]), maxTotalCostUsd: z.number().positive().max(100), expiresAt: z.string().datetime() }).strict();
const workspaceMatches = (request: NextRequest, workspaceId: string) => request.headers.get("x-workspace-id") === workspaceId;

export const GET = withStudioAuth<undefined>({ route: "/api/studio/model-routing/authorizations", action: "read", permission: "product:read" }, async (request, authz) => workspaceMatches(request, authz.workspaceId) ? noStoreJson({ success: true, items: await PRODUCTION_MODEL_ROUTING.listAuthorizations(authz.workspaceId) }) : noStoreJson({ success: false, code: "WORKSPACE_REQUIRED" }, { status: 400 }));

export const POST = withStudioAuth<undefined>({ route: "/api/studio/model-routing/authorizations", action: "write", permission: "product:content:write" }, async (request, authz) => {
