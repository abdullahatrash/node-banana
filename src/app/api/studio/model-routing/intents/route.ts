...
export const POST = withStudioAuth<undefined>({ route: "/api/studio/model-routing/intents", action: "write" }, async (request: NextRequest, authz) => {
  ...
  const status = result.kind === "invalid" ? 400 : result.kind === "fallback_not_authorized" || result.kind === "fallback_incompatible" || result.kind === "budget_denied" || result.kind === "conflict" ? 409 : result.kind === "budget_unavailable" || result.kind === "unavailable" ? 503 : 200;
