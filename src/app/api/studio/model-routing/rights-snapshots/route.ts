...
import { loadRightsEvidence } from "@/lib/model-routing/rights-evidence-repository";
...
export const POST = withStudioAuth<undefined>({ route: "/api/studio/model-routing/rights-snapshots", action: "write", permission: "product:content:write" }, async (request: NextRequest, authz) => {
  ...
  const evidence = await loadRightsEvidence(authz.workspaceId, [...new Set(parsed.data.evidenceIds)]); const sourceAssetIds = [...new Set(parsed.data.sourceAssetIds)];
  const validation = validateRightsEvidence({ workspaceId: authz.workspaceId, basis: parsed.data.basis, permittedRemix: parsed.data.permittedRemix, sourceAssetIds, evidence, at });
  if (!validation.ok) return noStoreJson({ success: false, code: validation.code }, { status: 422 });
  const rights = { basis: parsed.data.basis, permittedRemix: parsed.data.permittedRemix, evidence, sourceAssetIds };
  const value: InspirationRightsSnapshot = { schema: "inspiration-rights-snapshot/v1", id, workspaceId: authz.workspaceId, revision: 1, ...rights, digest: canonicalDigest(rights) as `sha256:${string}`, createdByUserId: authz.userId, createdAt: at };
