...
const payloadSchema = z.object({ id: z.string().min(1).max(200), status: z.enum(["succeeded","failed","canceled"]), version: z.string().min(1).max(200).nullable().optional() }).passthrough();
...
export async function POST(request: NextRequest) {
  ...
  const result = await recordReplicateCompletionWebhook({ database: getDb(), eventId: verified.eventId, predictionId: parsed.data.id, providerStatus: parsed.data.status, providerVersion: parsed.data.version ?? null, payload: raw, at });
