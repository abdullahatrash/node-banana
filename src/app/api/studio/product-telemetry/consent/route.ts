...
export const GET = withStudioAuth<undefined>({ route: "/api/studio/product-telemetry/consent", action: "read" }, async (_request, authz) => noStoreJson({ success: true, consent: await getReleaseControlService().getTelemetryConsent(authz.workspaceId, authz.userId) }));
export const POST = withStudioAuth<undefined>({ route: "/api/studio/product-telemetry/consent", action: "read" }, async (request: NextRequest, authz) => {
