...
vi.mock("@/lib/studio/authz", () => ({
  authorizeStudioRequest: mocks.authorize,
  authzErrorResponse: () => new Response("{}", { status: 403 }),
}));
vi.mock("@/lib/product-surfaces/repository", () => ({
  getProductRecord: vi.fn(),
  listProductRecords: mocks.list,
  createProductRecord: mocks.create,
  updateProductRecord: mocks.update,
  ProductRecordConflictError: class extends Error {},
  ProductRecordIdempotencyError: class extends Error {},
}));
...
const request = (body: unknown) =>
  new NextRequest("http://localhost/api/video-editor", {
    method: "POST",
...
describe("Workspace Video Editor drafts", () => {
  it("saves a main-video trim and reopens it through the public editor API", async () => {
    const response = await POST(
      request({ composition, idempotencyKey: "save-main-001" }),
    );
    ...
    mocks.list.mockResolvedValue([
      {
        ...
        revision: 1,
        kind: "content_piece",
        payload: { videoEditor: composition },
      },
    ]);
    const reopened = await GET(
      new NextRequest("http://localhost/api/video-editor"),
    );
    expect((await reopened.json()).records[0]).toMatchObject({
      id: saved.id,
      composition,
    });
  });
...
it("saves a timed Secondary video and rejects a segment longer than 15 seconds", async () => {
  ...
  const response = await POST(
    request({ composition: next, idempotencyKey: "two-video-001" }),
  );
  expect(response.status).toBe(200);
  expect((await response.json()).record.composition.secondary).toEqual(
    secondary,
  );
  const invalid = await POST(
    request({
      composition: { ...next, secondary: { ...secondary, trimEnd: 16 } },
      idempotencyKey: "two-video-002",
    }),
  );
...
it("saves independent audio layers but rejects foreign or wrong-kind media", async () => {
  ...
  const next = {
    ...composition,
    music,
    voiceover: { ...music, assetId: "voiceover", start: 0, muted: true },
  };
  mocks.asset.mockImplementation(async (_workspace, id) => ({
    id,
    type: id === "main" ? "video" : "audio",
    checksum: "sha256:verified",
    width: 1080,
    height: 1920,
    durationSeconds: 10,
  ...
  const response = await POST(
    request({ composition: next, idempotencyKey: "audio-layers-001" }),
  );
  expect(response.status).toBe(200);
  ...
  expect(
    (
      await POST(
        request({ composition: next, idempotencyKey: "audio-layers-002" }),
      )
    ).status,
  ).toBe(400);
});
