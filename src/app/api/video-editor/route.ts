...
const command = z.object({
  ...
  idempotencyKey: z.string().min(8).max(200), composition: compositionSchema,
}).strict().refine((value) => Boolean(value.id) === Boolean(value.expectedRevision));
const json = (body: unknown, status = 200) => NextResponse.json(body, { status, headers: { 'Cache-Control': 'no-store' } });
function project(record: { id: string; revision: number; payload: Record<string, unknown> }) {
...
export const POST = withStudioAuth<undefined>({ route: '/api/video-editor', action: 'write', permission: 'product:content:write' }, async (request, authz) => {
  const parsed = command.safeParse(await request.json().catch(() => null));
  if (!parsed.success || !parsed.data.composition.main) return json({ success: false, code: 'EDITOR_COMPOSITION_INVALID' }, 400);
  ...
  try {
    const input = { ...revision, workspaceId: authz.workspaceId, userId: authz.userId, title: composition.title, payload, state: 'draft' };
    const record = revision.id
      ? await updateProductRecord({ ...input, id: revision.id, expectedRevision: revision.expectedRevision!, expectedKind: 'content_piece', beforeUpdate: async (_tx, current) => { if (current.archivedAt || !current.payload.videoEditor) throw new ProductRecordConflictError('Editor draft is unavailable'); } })
      : await createProductRecord({ ...input, kind: 'content_piece' });
    return record ? json({ success: true, record: project(record) }) : json({ success: false, code: 'EDITOR_NOT_FOUND' }, 404);
  } catch (error) {
    if (error instanceof ProductRecordConflictError || error instanceof ProductRecordIdempotencyError) return json({ success: false, code: 'EDITOR_SAVE_CONFLICT' }, 409);
    throw error;
  }
});
