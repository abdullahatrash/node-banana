...
import { z } from "zod";
import { activateCampaignCommand, CampaignRuntimeError, pauseCampaignCommand } from "@/lib/product-surfaces/campaign-runtime";
  ...
  try {
    const record = parsed.data.action === "save_draft"
      ...
      : parsed.data.action === "activate"
        ...
        : await pauseCampaignCommand({ workspaceId: authz.workspaceId, userId: authz.userId, ...parsed.data });
