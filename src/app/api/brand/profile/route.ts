...
import { activateBrandRevision, BrandRevisionConflictError, createBrandRevision, refreshBrandSource } from "@/lib/product-surfaces/brand";
...
export const POST = withStudioAuth<undefined>({ route: "/api/brand/profile", action: "write", permission: "workspaces:write" }, async (request, authz) => {
  ...
  try {
    const result = parsed.data.action === "create_revision"
      ...
      : parsed.data.action === "activate_revision"
        ? await activateBrandRevision({ workspaceId: authz.workspaceId, userId: authz.userId, ...parsed.data })
        : await refreshBrandSource({ workspaceId: authz.workspaceId, userId: authz.userId, ...parsed.data });
    return NextResponse.json({ success: true, ...result }, { status: parsed.data.action === "create_revision" ? 201 : parsed.data.action === "refresh_source" ? 202 : 200 });
  } catch (error) {
