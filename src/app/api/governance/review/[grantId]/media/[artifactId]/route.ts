...
import { openReviewMediaToken, reauthorizeReviewMediaAccess } from "@/lib/governance/review-presentation";
...
export async function GET(request: NextRequest, context: { params: Promise<{ grantId: string; artifactId: string }> }) {
  ...
  try {
    if (!await reauthorizeReviewMediaAccess(payload, PRODUCTION_GOVERNANCE_REPOSITORY)) {
