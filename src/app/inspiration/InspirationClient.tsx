...
import { BookmarkPlus, ExternalLink, Filter, LoaderCircle, Plus, RefreshCw, Search, Sparkles, TrendingUp } from "lucide-react";
...
export function InspirationClient({ items, performanceSources = [] }: { items: InspirationDiscoveryItem[]; performanceSources?: WorkspaceOwnedPerformanceSource[] }) {
  ...
  return <div className="space-y-7">
    ...
    <details className="rounded-3xl border bg-card" open={performanceSources.length > 0 && items.length === 0 ? true : undefined}><summary className="cursor-pointer list-none p-6 text-xl font-semibold"><span className="inline-flex items-center gap-2"><TrendingUp className="size-5 text-amber-600" />{t("performance.title")}</span></summary><div className="border-t p-6"><p className="mb-5 text-sm text-muted-foreground">{performanceSources.length ? t("performance.description") : t("performance.empty")}</p>{performanceSources.length > 0 && <form onSubmit={recordPerformance} className="grid gap-4 md:grid-cols-2"><label className="grid gap-2 text-sm font-medium md:col-span-2">{t("performance.fields.post")}<select name="performanceSource" required className="min-h-11 rounded-xl border bg-background px-3"><option value="">{t("performance.selectPost")}</option>{performanceSources.map((source) => <option key={source.id} value={source.id}>{source.title} · {source.channel}</option>)}</select></label><Input name="performanceSourceRef" label={t("performance.fields.sourceRef")} /><Input name="performanceObservedAt" label={t("performance.fields.observedAt")} type="datetime-local" /><Input name="performanceViews" label={t("performance.fields.views")} type="number" min="0" /><Input name="performanceLikes" label={t("performance.fields.likes")} type="number" min="0" /><Input name="performanceComments" label={t("performance.fields.comments")} type="number" min="0" required={false} /><Input name="performanceRegion" label={t("performance.fields.region")} /><label className="grid gap-2 text-sm font-medium">{t("performance.fields.language")}<select name="performanceLanguage" className="min-h-11 rounded-xl border bg-background px-3"><option value="ar">{t("languages.ar")}</option><option value="en">{t("languages.en")}</option></select></label><label className="grid gap-2 text-sm font-medium">{t("performance.fields.variety")}<select name="performanceArabicVariety" className="min-h-11 rounded-xl border bg-background px-3"><option value="">{t("performance.noVariety")}</option>{ARABIC_VARIETIES.map((value) => <option key={value} value={value}>{t(`performance.varieties.${value}`)}</option>)}</select></label><label className="grid gap-2 text-sm font-medium">{t("performance.fields.format")}<select name="performanceFormat" className="min-h-11 rounded-xl border bg-background px-3">{CONTENT_FORMATS.map((value) => <option value={value} key={value}>{t(`formats.${value}`)}</option>)}</select></label><Input name="performanceTags" label={t("performance.fields.tags")} required={false} /><button disabled={busy} className="inline-flex min-h-11 items-center justify-center gap-2 self-end rounded-xl bg-amber-300 px-4 font-semibold text-stone-950 disabled:opacity-50">{busy ? <LoaderCircle className="size-4 animate-spin" /> : <TrendingUp className="size-4" />}{t("performance.submit")}</button></form>}</div></details>
...
function Input({ name, label, required = true, type = "text", min }: { name: string; label: string; required?: boolean; type?: string; min?: string }) { return <label className="grid gap-2 text-sm font-medium">{label}<input dir={type === "number" || type === "datetime-local" ? "ltr" : "auto"} name={name} type={type} min={min} step={type === "number" ? "1" : undefined} required={required} className="min-h-11 rounded-xl border bg-background px-3 font-normal" /></label>; }
