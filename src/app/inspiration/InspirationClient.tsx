...
function LanguageAndVariety({ prefix, t }: { prefix: "sync" | "performance"; t: (key: string) => string }) { return <><label className="grid gap-2 text-sm font-medium">{t("performance.fields.language")}<select name={`${prefix}Language`} className="min-h-11 rounded-xl border bg-background px-3"><option value="ar">{t("languages.ar")}</option><option value="en">{t("languages.en")}</option></select></label><label className="grid gap-2 text-sm font-medium">{t("performance.fields.variety")}<select name={`${prefix}ArabicVariety`} className="min-h-11 rounded-xl border bg-background px-3"><option value="">{t("performance.noVariety")}</option>{ARABIC_VARIETIES.map((value) => <option key={value} value={value}>{t(`performance.varieties.${value}`)}</option>)}</select></label></>; }
