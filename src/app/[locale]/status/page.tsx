...
export default async function StatusPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  if (isDatabaseConfigured()) { try { incidents = await getReleaseControlService().publicIncidents(locale); status = incidents.some((item) => item.status !== "resolved") ? "degraded" : "operational"; } catch {} }
