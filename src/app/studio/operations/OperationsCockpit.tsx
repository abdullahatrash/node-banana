...
export function OperationsCockpit() {
  ...
  return <main className="mx-auto grid w-full max-w-7xl gap-6 p-4 lg:grid-cols-[minmax(0,1fr)_24rem] lg:p-6">
    <section aria-labelledby="operations-title"><header className="flex flex-wrap items-start justify-between gap-3"><div><h1 id="operations-title" className="text-2xl font-semibold">{t("title")}</h1><p className="mt-1 text-sm text-muted-foreground">{t("description")}</p></div><Button variant="outline" onClick={() => void load()}><RefreshCwIcon className="size-4" />{t("refresh")}</Button></header>
      ...
      {nextCursor ? <Button className="mt-4" variant="outline" onClick={() => void load(nextCursor)}>{t("loadMore")}</Button> : null}
      {!items.length && !error ? <p className="mt-6 rounded-xl border border-dashed p-8 text-center text-muted-foreground">{t("empty")}</p> : null}
    </section>
    <aside aria-label={t("details")} className="rounded-xl border bg-card p-4">{selected ? <><h2 className="text-lg font-semibold">{t("details")}</h2><p className="mt-1 break-all font-mono text-xs" dir="ltr">{selected.id}</p><div className="mt-4 flex flex-wrap gap-2">{controls.cancel && !isTerminalOperationState(selected.state) && selected.state !== "outcome_unknown" ? <Button disabled={busy} variant="destructive" onClick={() => void mutate({ action: "cancel", expectedRevision: selected.revision })}><XCircleIcon className="size-4" />{t("cancel")}</Button> : null}{controls.retry ? <Button disabled={busy} variant="outline" onClick={() => void mutate({ action: "retry" })}><RotateCcwIcon className="size-4" />{t("retry")}</Button> : null}</div><h3 className="mt-5 text-sm font-semibold">{t("evidence")}</h3><dl className="mt-2 grid gap-2 text-xs">{Object.entries(selected.metadata).map(([key, value]) => <div key={key} className="rounded-md bg-muted/40 p-2"><dt className="font-medium">{key}</dt><dd className="mt-1 break-all text-muted-foreground" dir="auto">{Array.isArray(value) ? value.join(", ") : String(value ?? "—")}</dd></div>)}</dl><ol className="mt-5 space-y-3">{events.map((event) => <li key={event.id} className="border-s ps-3 text-sm"><strong>{t(`states.${event.to}`)}</strong><p className="text-xs text-muted-foreground" dir="auto">{event.reasonCode}</p></li>)}</ol></> : <p className="text-sm text-muted-foreground">{t("select")}</p>}</aside>
  </main>;
}
