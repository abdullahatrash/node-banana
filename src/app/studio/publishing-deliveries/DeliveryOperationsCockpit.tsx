...
export function DeliveryOperationsCockpit({ deliveryId }: { deliveryId: string }) {
  ...
  if (loading && !delivery) {
    return <main aria-busy="true" className="min-h-screen bg-neutral-950 p-8 text-neutral-100"><p role="status">Loading canonical Delivery evidence…</p></main>;
  }
  if (authLost) {
    return <main className="min-h-screen bg-neutral-950 p-8 text-neutral-100"><h1 className="text-2xl font-semibold">Delivery operations unavailable</h1><p role="alert" className="mt-3 text-amber-200">Authorization was lost. Protected Delivery evidence was cleared and live updates stopped.</p></main>;
  }
    ...
    <main className="min-h-screen bg-neutral-950 px-5 py-8 text-neutral-100">
      <div className="mx-auto max-w-7xl space-y-6">
        <header className="flex flex-wrap items-start justify-between gap-4">
          <div><p className="text-xs font-semibold uppercase tracking-[0.24em] text-sky-400">Canonical publishing operations</p><h1 className="mt-2 text-3xl font-semibold">Publishing Delivery</h1><p className="mt-2 font-mono text-sm text-neutral-400">{deliveryId}</p></div>
          <button type="button" onClick={() => void reload()} className="rounded border border-neutral-700 px-4 py-2 text-sm focus:ring-2 focus:ring-sky-400">Reload canonical view</button>
        </header>
        ...
        {delivery ? <section aria-label="Delivery canonical state" className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{[
        ...
        <Panel title="Delivery target, schedule, and validation" error={errors.delivery}>
          {delivery ? <div className="mt-4 grid gap-4 lg:grid-cols-2"><dl className="space-y-2 text-sm"><div><dt className="text-neutral-500">Plan Revision</dt><dd>{delivery.planRevision} · <code>{delivery.planRevisionId}</code></dd></div><div><dt className="text-neutral-500">Approval</dt><dd><code>{delivery.approvalRequestId}</code> · decision <code>{delivery.approvalDecisionId}</code></dd></div><div><dt className="text-neutral-500">Artifacts</dt><dd>{delivery.artifactIds.join(", ")}</dd></div><div><dt className="text-neutral-500">Schedule</dt><dd>{delivery.targetSnapshot.target.timing.kind} · {delivery.publishAt}</dd></div><div><dt className="text-neutral-500">Validation blockers</dt><dd>{delivery.targetSnapshot.validation.blockerCodes.length ? delivery.targetSnapshot.validation.blockerCodes.join(", ") : "None at immutable validation"}</dd></div></dl><details><summary className="cursor-pointer font-medium">Exact immutable Target Snapshot</summary><pre className="mt-2 max-h-96 overflow-auto whitespace-pre-wrap rounded bg-neutral-950 p-3 text-xs">{safeJson(delivery.targetSnapshot)}</pre></details></div> : <p className="mt-3 text-neutral-500">Unknown</p>}
        </Panel>

        <div className="grid gap-6 xl:grid-cols-2">
          <Panel title="Exact Publishing Plan Revision" error={errors.plan}>{plan ? <details open className="mt-4"><summary className="cursor-pointer">Revision {plan.revision} · {plan.definitionDigest}</summary><pre className="mt-2 max-h-96 overflow-auto whitespace-pre-wrap rounded bg-neutral-950 p-3 text-xs">{safeJson(plan)}</pre></details> : <p className="mt-3 text-neutral-500">Unknown</p>}</Panel>
          <Panel title="Exact Publishing Approval" error={errors.approval}>{approval ? <div className="mt-4 text-sm"><p>Status <strong>{approval.approval.status}</strong> · inspection <code>{approval.approval.inspectionDigest}</code></p><p className="mt-2">Decision {approval.approval.decision?.decision ?? "Unknown"} · consumed {approval.approval.consumption ? "yes" : "no"}</p><details className="mt-3"><summary className="cursor-pointer">Approval binding and evidence</summary><pre className="mt-2 max-h-80 overflow-auto whitespace-pre-wrap rounded bg-neutral-950 p-3 text-xs">{safeJson(approval.approval)}</pre></details></div> : <p className="mt-3 text-neutral-500">Unknown</p>}</Panel>
        </div>

        <Panel title="Effect and cancellation truth">
          {delivery ? <div className="mt-4 grid gap-4 lg:grid-cols-2"><dl className="space-y-2 text-sm"><div><dt className="text-neutral-500">Effect Key</dt><dd><code>{delivery.effectKey}</code></dd></div><div><dt className="text-neutral-500">Intent / Adapter contract</dt><dd><code>{display(delivery.intentDigest)}</code><br /><code>{display(delivery.providerAdapterContractDigest)}</code></dd></div><div><dt className="text-neutral-500">Provider operation</dt><dd>{display(delivery.providerOperationRef)}</dd></div><div><dt className="text-neutral-500">Latest effect evidence</dt><dd><code>{display(delivery.latestEffectEvidenceDigest)}</code></dd></div><div><dt className="text-neutral-500">Failure</dt><dd>{display(delivery.failureCode)} · {display(delivery.failureClass)} · retryable {display(delivery.failureRetryable)}</dd></div><div><dt className="text-neutral-500">Durable cancellation record</dt><dd>{cancellation ? <><strong>{cancellation.outcome}</strong> from state {cancellation.stateAtRequest}; externally completed at request {display(cancellation.externallyCompletedAtRequest)}; durable {String(cancellation.durable)}; externally reversed {String(cancellation.externallyReversed)}</> : "No cancellation request is recorded."}</dd></div></dl><div className="rounded bg-neutral-950 p-4"><p className="text-sm text-neutral-400">Actions re-read current canonical state first. A concurrent Agent change stops the action and refreshes this screen.</p><div className="mt-4 flex flex-wrap gap-3"><button type="button" disabled={!futureCancellable || busyAction !== null} onClick={() => void cancelDelivery()} className="rounded bg-red-300 px-4 py-2 text-sm font-medium text-red-950 disabled:opacity-40">Cancel future Delivery</button><button type="button" disabled={!ambiguous || !delivery.latestEffectEvidenceDigest || busyAction !== null} onClick={() => void reconcileDelivery()} className="rounded bg-amber-300 px-4 py-2 text-sm font-medium text-amber-950 disabled:opacity-40">Request reconciliation</button></div>{ambiguous ? <p className="mt-3 text-sm text-amber-200">Provider outcome is ambiguous. Unknown is not failure or success; reconciliation observes the same Effect Key without a new public effect.</p> : null}</div></div> : null}
        </Panel>

        <div className="grid gap-6 xl:grid-cols-2">
          <Panel title="Budget reservations and spend warnings" error={errors.budget}><div className="mt-4 space-y-3 text-sm">{budgetPolicies.map(({ policy, revision }) => <article key={policy.id} className="rounded bg-neutral-950 p-3">{policy.scope} · {policy.currency} · warning {revision.warningThreshold} · hard limit {revision.hardLimit} · unknown pricing {revision.unknownPriceTreatment}</article>)}{budgetReservations.map((item) => <article key={item.id} className="rounded bg-neutral-950 p-3"><code>{item.id}</code> · Run <code>{item.runId}</code> · policy <code>{item.policyRevisionId}</code><br />{item.state} · reserved {item.reservedAmount} {item.currency} · held {item.heldAmount} · settled {item.settledAmount} · released {item.releasedAmount}<br /><span className="text-neutral-500">Period {item.period.kind} · {item.period.startsAt} → {display(item.period.endsAt)}</span></article>)}{budgetStatus ? <><p className="text-xs text-neutral-500">Canonical status evaluated {budgetStatus.evaluatedAt}.</p>{budgetStatus.items.map((item) => <article key={`${item.scope}:${item.policyRevisionId}`} className="rounded border border-neutral-800 bg-neutral-950 p-3"><strong>{item.warningState.replaceAll("_", " ")}</strong> · {item.committed} / {item.hardLimit} {item.currency} committed · {item.available} available<br /><span className={item.certainty === "known" ? "text-neutral-400" : "text-amber-200"}>{item.certainty.replaceAll("_", " ")} · unknown-cost Reservations {item.unknownReservationCount}</span><br /><span className="text-neutral-500">{item.scope} policy <code>{item.policyRevisionId}</code> · period {item.period.kind} {item.period.startsAt} → {display(item.period.endsAt)}</span></article>)}</> : <p className="text-neutral-500">Current spend-warning truth: Unknown. No canonical Budget Status is available.</p>}</div></Panel>
          <Panel title="Effective Quotas and reservations" error={errors.quota}><div className="mt-4 space-y-3 text-sm">{quotaCapacities.map((item) => <article key={item.policy.id} className="rounded bg-neutral-950 p-3">{item.policy.dimension} · committed {item.committed} {item.policy.unit} · available {item.available} · warning {display(item.warning)} · exhausted {display(item.exhausted)}</article>)}{quotaReservations.map((item) => <article key={item.id} className="rounded bg-neutral-950 p-3"><code>{item.id}</code> · Run <code>{display(item.runId)}</code> · policy <code>{item.policyRevisionId}</code><br />{item.state} · {item.dimension} · reserved {item.reservedAmount} {item.unit} · overage {item.overageAmount}<br /><span className="text-neutral-500">Window {item.window.kind} · {safeJson(item.window)}</span></article>)}<p className="text-xs text-neutral-500">Showing up to 100 Reservation rows; this capability does not expose continuation.</p></div></Panel>
        </div>

        <Panel title="Quota Wait eligibility and resumptions" error={errors.waits}><div className="mt-4 space-y-3">{quotaWaits.map((wait) => <article key={wait.id} className="rounded bg-neutral-950 p-4 text-sm"><div className="flex flex-wrap items-center justify-between gap-3"><p><strong>{wait.state}</strong> · {wait.boundary} · eligible {display(wait.eligibleAt)}</p>{wait.state === "waiting" ? <button type="button" disabled={busyAction !== null} onClick={() => void resumeWait(wait.id)} className="rounded border border-sky-700 px-3 py-2 text-sky-200 disabled:opacity-40">Re-evaluate and resume</button> : null}</div><p className="mt-2"><code>{wait.id}</code> · Run <code>{wait.runId}</code> · transition <code>{wait.transitionKey}</code></p><p className="mt-2 text-neutral-400">Subject {wait.subject.kind} <code>{wait.subject.id}</code> · {wait.reasonCode}</p><ul className="mt-2 list-disc pl-5 text-neutral-300">{wait.claims.map((claim) => <li key={`${claim.dimension}:${claim.unit}`}>Requests {claim.amount} {claim.unit} of {claim.dimension}</li>)}</ul><p className="mt-2 text-neutral-400">Resume reason {display(wait.resumeReason)} · actor {wait.resumedBy ? safeJson(wait.resumedBy) : "Unknown"} · Reservations {wait.resolutionReservationIds.join(", ") || "None"}</p><details className="mt-3"><summary className="cursor-pointer">Canonical eligibility evidence ({wait.evidence.length})</summary><pre className="mt-2 max-h-96 overflow-auto whitespace-pre-wrap rounded border border-neutral-800 p-3 text-xs text-neutral-400">{safeJson(wait.evidence)}</pre></details></article>)}{quotaWaits.length === 0 ? <p className="text-sm text-neutral-500">No Quota Wait evidence for this Delivery principal.</p> : null}<p className="text-xs text-neutral-500">Showing up to 100 Wait rows; this capability does not expose continuation.</p></div></Panel>

        <Panel title="Emergency Spend Suspension" error={errors.spend}><div className="mt-4 grid gap-4 lg:grid-cols-2"><div><p className={`text-lg font-medium ${spendControl?.suspended ? "text-red-200" : "text-emerald-200"}`}>{spendControl ? (spendControl.suspended ? "Suspended" : "Not suspended") : "Unknown"}</p><p className="mt-2 text-sm text-neutral-400">Suspension blocks new and not-yet-started provider spend. Removing it does not bypass canonical readiness, Quota, Budget, or Approval checks.</p>{spendControl ? <dl className="mt-3 space-y-1 text-sm"><div><dt className="inline text-neutral-500">Revision </dt><dd className="inline">{spendControl.revision}</dd></div><div><dt className="inline text-neutral-500">Reason </dt><dd className="inline">{display(spendControl.reason)}</dd></div><div><dt className="inline text-neutral-500">Actor / recorded </dt><dd className="inline">{display(spendControl.actorUserId)} · {display(spendControl.recordedAt)}</dd></div><div><dt className="inline text-neutral-500">Policy event / authorization evidence </dt><dd className="inline"><code>{display(spendControl.policyEventId)}</code> · <code>{display(spendControl.authorizationEvidenceRef)}</code></dd></div></dl> : null}</div><form onSubmit={(event) => void changeSuspension(event, !spendControl?.suspended)} className="space-y-3"><label className="block text-sm">Durable policy reason<textarea name="reason" required minLength={1} maxLength={500} className="mt-1 min-h-24 w-full rounded border border-neutral-700 bg-neutral-950 p-3" /></label><button disabled={!spendControl || busyAction !== null} className="rounded bg-sky-300 px-4 py-2 font-medium text-sky-950 disabled:opacity-40">{spendControl?.suspended ? "Remove Emergency Spend Suspension" : "Enable Emergency Spend Suspension"}</button></form></div></Panel>

        <Panel title="Ordered retained Delivery Events" error={errors.events}><ol className="mt-4 space-y-3">{events.map((event) => <li key={event.id} className="border-l border-neutral-700 pl-3 text-sm"><p><strong>#{event.sequence}</strong> · {event.type} <time className="text-neutral-500">{event.occurredAt}</time></p><pre className="mt-1 overflow-auto whitespace-pre-wrap text-xs text-neutral-500">{safeJson(event.evidence)}</pre></li>)}</ol><button type="button" onClick={() => void poll()} className="mt-4 rounded border border-neutral-700 px-3 py-2 text-sm">Check for concurrent changes</button></Panel>

        {actionEvidence ? <Panel title="Latest canonical action result"><pre className="mt-4 max-h-80 overflow-auto whitespace-pre-wrap rounded bg-neutral-950 p-3 text-xs">{safeJson(actionEvidence)}</pre></Panel> : null}
      </div>
    </main>
  );
}
