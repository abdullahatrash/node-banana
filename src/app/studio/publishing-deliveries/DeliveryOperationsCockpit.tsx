  ...
  return (
    <main className="min-h-screen bg-neutral-950 px-5 py-8 text-neutral-100">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <Panel title={t("copy.orderedRetainedDeliveryEvents")} error={errors.events}><ol className="mt-4 space-y-3">{events.map((event) => <li key={event.id} className="border-l border-neutral-700 pl-3 text-sm"><p><strong>#{event.sequence}</strong> · {event.type} <time className="text-neutral-500">{event.occurredAt}</time></p><pre className="mt-1 overflow-auto whitespace-pre-wrap text-xs text-neutral-500">{safeJson(event.evidence)}</pre></li>)}</ol><button type="button" onClick={() => void poll()} className="mt-4 rounded border border-neutral-700 px-3 py-2 text-sm">{t("copy.checkForConcurrentChanges")}</button></Panel>
