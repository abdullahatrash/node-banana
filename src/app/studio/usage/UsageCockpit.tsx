      ...
      <div className="mx-auto max-w-7xl space-y-8">
        ...
        <section aria-labelledby="usage-overview" className="grid gap-4 md:grid-cols-3">
          ...
          <article className="rounded-xl border border-neutral-800 bg-neutral-900 p-5 md:col-span-2">
            ...
            <div className="mt-2 flex flex-wrap gap-4">{costs.length ? costs.map((cost) => <span key={value(cost, "currency")} className="font-mono text-xl">{value(cost, "amount")} {value(cost, "currency")}</span>) : <span className="text-neutral-400">{t("noKnownCost")}</span>}</div>
          </article>
        </section>

        <section className="rounded-xl border border-neutral-800 bg-neutral-900 p-5">
          <h2 className="text-lg font-medium">{t("quantityTotals")}</h2>
          <div className="mt-4 grid gap-3 md:grid-cols-3">{quantities.map((item) => <div key={`${value(item, "dimension")}:${value(item, "source")}`} className="rounded-lg bg-neutral-950 p-3"><p className="text-xs text-neutral-500">{value(item, "dimension")} · {value(item, "source")}</p><p className="mt-1 font-mono">{value(item, "quantity")} {value(item, "unit")}</p>{Number(item.unknownCount ?? 0) > 0 ? <p className="mt-1 text-xs text-amber-300">{t("unknownCount", { count: String(item.unknownCount) })}</p> : null}</div>)}</div>
        </section>

        <section className="grid gap-6 xl:grid-cols-2">
          <div className="rounded-xl border border-neutral-800 bg-neutral-900 p-5"><h2 className="text-lg font-medium">{t("records")}</h2><div className="mt-4 space-y-3">{usage.map((item) => { const binding = item.binding as JsonRecord | undefined; return <article key={value(item, "id")} className="rounded-lg bg-neutral-950 p-3 text-sm"><div className="flex justify-between gap-3"><code>{value(item, "dimension")}</code><span className={item.quantity === null ? "text-amber-300" : "text-emerald-300"}>{value(item, "quantity")}</span></div><p className="mt-2 text-xs text-neutral-500">{t("recordBinding", { run: binding ? value(binding, "runId") : "—", source: value(item, "source"), artifact: value(item, "directArtifactId") })}</p></article>; })}</div>{usageCursor ? <button type="button" onClick={() => void loadMoreUsage().catch((cause) => setError(String(cause)))} className="mt-4 rounded-md border border-neutral-700 px-3 py-2 text-sm hover:bg-neutral-800">{t("loadMoreUsage")}</button> : null}</div>
          <div className="rounded-xl border border-neutral-800 bg-neutral-900 p-5"><h2 className="text-lg font-medium">{t("valuations")}</h2><div className="mt-4 space-y-3">{valuations.map((item) => <article key={value(item, "id")} className="rounded-lg bg-neutral-950 p-3 text-sm"><div className="flex justify-between gap-3"><span>{value(item, "basis")} · {value(item, "pricingSource")}</span><code className={item.amount === null ? "text-amber-300" : "text-emerald-300"}>{value(item, "amount")} {item.currency ? value(item, "currency") : ""}</code></div><p className="mt-2 text-xs text-neutral-500">{t("valuationBinding", { snapshots: Array.isArray(item.pricingSnapshotIds) ? item.pricingSnapshotIds.length : 0, supersedes: value(item, "supersedesCostValuationId") })}</p></article>)}</div>{valuationCursor ? <button type="button" onClick={() => void loadMoreValuations().catch((cause) => setError(String(cause)))} className="mt-4 rounded-md border border-neutral-700 px-3 py-2 text-sm hover:bg-neutral-800">{t("loadMoreValuations")}</button> : null}</div>
        </section>

        <section className="grid gap-6 xl:grid-cols-2">
          <div className="rounded-xl border border-neutral-800 bg-neutral-900 p-5"><h2 className="text-lg font-medium">{t("events")}</h2><ol className="mt-4 space-y-2">{events.map((item) => <li key={value(item, "id")} className="border-s border-neutral-700 ps-3 text-sm"><span>{value(item, "type")}</span><time className="ms-2 text-xs text-neutral-500">{value(item, "occurredAt")}</time></li>)}</ol>{eventCursor ? <button type="button" onClick={() => void loadMoreEvents().catch((cause) => setError(String(cause)))} className="mt-4 rounded-md border border-neutral-700 px-3 py-2 text-sm hover:bg-neutral-800">{t("loadMoreEvents")}</button> : null}</div>
          <div className="rounded-xl border border-neutral-800 bg-neutral-900 p-5"><h2 className="text-lg font-medium">{t("agentTitle")}</h2><form onSubmit={loadAgent} className="mt-4 flex gap-2"><input name="principalId" aria-label={t("principalId")} required className="min-w-0 flex-1 rounded-md border border-neutral-700 bg-neutral-950 px-3 py-2 text-sm" placeholder={t("principalPlaceholder")} dir="ltr" /><button className="rounded-md bg-emerald-500 px-4 py-2 text-sm font-medium text-neutral-950">{t("load")}</button></form>{agentView ? <pre className="mt-4 max-h-72 overflow-auto rounded-lg bg-neutral-950 p-3 text-xs text-neutral-300">{JSON.stringify(agentView, null, 2)}</pre> : <p className="mt-3 text-sm text-neutral-500">{t("agentDescription")}</p>}</div>
        </section>
      </div>
    </main>
  );
