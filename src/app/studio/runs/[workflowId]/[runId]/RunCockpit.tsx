    ...
    <main className="min-h-screen bg-neutral-950 px-5 py-8 text-neutral-100">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <Panel title={t("copy.immutableWorkflowRevision")} error={errors.revision}>
          {revision ? <div className="mt-4 space-y-3 text-sm"><p><span className="text-neutral-500">{t("copy.revision")}</span> {display(revision.revision)} · <code>{display(revision.definitionDigest)}</code></p><p><span className="text-neutral-500">{t("copy.operationRegistry")}</span> <code>{display(revision.operationRegistryDigest)}</code></p><ol className="space-y-2">{run?.startSnapshot.operationContracts.map((item) => <li key={item.stepId} className="rounded-lg bg-neutral-950 p-3"><strong>{item.stepId}</strong> · {item.identity}<br /><code className="text-xs text-neutral-500">{item.contractDigest}</code></li>)}</ol><details><summary className="cursor-pointer font-medium">{t("copy.exactImmutableDefinition")}</summary><pre className="mt-2 max-h-96 overflow-auto whitespace-pre-wrap rounded bg-neutral-950 p-3 text-xs">{JSON.stringify(revision.definition, null, 2)}</pre></details></div> : <p className="mt-3 text-neutral-500">{t("copy.revisionEvidenceIsLoading")}</p>}
        </Panel>

        <Panel title={t("copy.canonicalRunSnapshots")}>
          {run ? <div className="mt-4 grid gap-4 lg:grid-cols-2"><details open><summary className="cursor-pointer font-medium">{t("copy.retainedStartSnapshot")}</summary><pre className="mt-2 max-h-96 overflow-auto whitespace-pre-wrap rounded bg-neutral-950 p-3 text-xs">{JSON.stringify({ schema: run.startSnapshot.schema, workflowId: run.startSnapshot.workflowId, workflowRevisionId: run.startSnapshot.workflowRevisionId, workflowRevision: run.startSnapshot.workflowRevision, definitionDigest: run.startSnapshot.definitionDigest, operationRegistryDigest: run.startSnapshot.operationRegistryDigest, inputs: run.startSnapshot.inputs, operationContracts: run.startSnapshot.operationContracts, artifactReferences: run.startSnapshot.artifactReferences, providerResolutions: run.startSnapshot.providerResolutions ?? [] }, null, 2)}</pre></details><details open><summary className="cursor-pointer font-medium">{t("copy.retainedFinalSnapshot")}</summary>{run.finalSnapshot ? <pre className="mt-2 max-h-96 overflow-auto whitespace-pre-wrap rounded bg-neutral-950 p-3 text-xs">{JSON.stringify(run.finalSnapshot, null, 2)}</pre> : <p className="mt-2 text-sm text-neutral-500">{t("copy.unknownNoFinalSnapshotIsDurableYet")}</p>}</details></div> : <p className="mt-3 text-neutral-500">{t("copy.runSnapshotEvidenceIsUnavailable")}</p>}
        </Panel>

        {run?.derivation ? <Panel title={t("copy.retryLineage")}><div className="mt-3 text-sm"><p>{t("copy.derivedFromRun")} <code>{run.derivation.sourceRunId}</code>{t("copy.root")} <code>{run.derivation.rootRunId}</code>{t("copy.fromStep")} <strong>{run.derivation.retryFromStepId}</strong>.</p><p className="mt-2 text-neutral-400">{run.derivation.reusedOutputs.length} {t("copy.priorStepOutputSetSReusedWithout")}</p></div></Panel> : null}

        <Panel title={t("copy.stepAttempts")} error={errors.attempts}>
          <div className="mt-4 overflow-x-auto"><table className="w-full text-start text-sm"><thead className="text-neutral-500"><tr><th scope="col" className="pb-2">{t("copy.stepAttempt")}</th><th scope="col">{t("copy.state")}</th><th scope="col">{t("copy.operation")}</th><th scope="col">{t("copy.effectKey")}</th><th scope="col">{t("copy.outcome")}</th></tr></thead><tbody>{attempts.map((attempt) => <tr key={attempt.id} className="border-t border-neutral-800"><th scope="row" className="py-3">{attempt.stepId} / {attempt.attempt}</th><td>{attempt.state}</td><td>{attempt.provider} · {attempt.model}</td><td><code className="break-all text-xs">{attempt.effectKey}</code></td><td>{attempt.outcome?.kind ?? "In progress"}{attempt.failureCode ? ` · ${attempt.failureCode}` : ""}</td></tr>)}</tbody></table>{attempts.length === 0 ? <p className="py-4 text-neutral-500">{t("copy.noStepAttemptHasStartedTheAccepted")}</p> : null}</div>
        </Panel>

        <Panel title={t("copy.inputAndOutputArtifacts")} error={errors.artifacts}>
          <div className="mt-4 grid gap-3 lg:grid-cols-2">{artifacts.map(({ artifact, textContent }) => <article key={artifact.id} className="rounded-lg bg-neutral-950 p-4 text-sm"><div className="flex justify-between gap-3"><strong>{inputArtifactIds.has(artifact.id) ? "Input" : "Output"} · {artifact.kind}</strong><span>{artifact.sizeBytes} {t("copy.bytes")}</span></div><p className="mt-2 break-all text-xs text-neutral-500">{artifact.id} · {artifact.digest}</p><p className="mt-2">{artifact.mediaType} {t("copy.origin")} {artifact.origin.kind}</p>{artifact.origin.kind === "generated" ? <p className="mt-2 text-xs text-neutral-400">{t("copy.revision")} {artifact.origin.workflowRevision.revision} {t("copy.run")} {artifact.origin.run.runId} {t("copy.attempt")} {artifact.origin.stepAttempt.stepAttemptId} {t("copy.effect")} <code>{artifact.origin.effectKey}</code></p> : null}<ul className="mt-2 list-disc ps-5 text-xs text-neutral-400">{artifact.lineage.inputs.map((input, index) => <li key={`${input.port}:${index}`}>{input.port} ← {input.source.kind} · {display(input.artifactId)}</li>)}</ul>{textContent ? <p className="mt-3 max-h-28 overflow-auto whitespace-pre-wrap rounded bg-neutral-900 p-2">{textContent}</p> : null}</article>)}</div>
        </Panel>

        <div className="grid gap-6 xl:grid-cols-2">
          <Panel title={t("copy.usageRecords")} error={errors.usage}><div className="mt-4 space-y-3">{usage.map((item) => <article key={display(item.id)} className="rounded-lg bg-neutral-950 p-3 text-sm"><div className="flex justify-between gap-3"><code>{display(item.dimension)}</code><span className={item.quantity == null ? "text-amber-200" : "text-emerald-200"}>{display(item.quantity)} {display(item.unit)}</span></div><p className="mt-1 text-xs text-neutral-500">{t("copy.source")} {display(item.source)} {t("copy.directChargeAttribution")} {display(item.directArtifactId)}</p><p className="mt-1 text-xs text-neutral-500">{t("copy.nonChargeLineageContext")} {Array.isArray(item.lineageArtifactIds) && item.lineageArtifactIds.length ? item.lineageArtifactIds.join(", ") : "None"}</p>{item.supersedesUsageRecordId ? <p className="mt-1 text-xs">{t("copy.correctionSupersedes")} {display(item.supersedesUsageRecordId)}</p> : null}</article>)}</div>{usageCursor ? <button type="button" onClick={() => void loadMoreUsage()} className="mt-4 rounded border border-neutral-700 px-3 py-2 text-sm">{t("copy.loadMoreUsage")}</button> : null}</Panel>
        ...
        <Panel title={t("copy.usageCertainty")} error={errors.usageSummary}><div className="mt-4 text-sm"><p className={usageSummary?.complete === true ? "text-emerald-200" : "text-amber-200"}>{usageSummary?.complete === true ? "All applicable values are known." : "Contains unknown usage or pricing; unknown is not zero."}</p><div className="mt-3 flex flex-wrap gap-3">{usageSummary?.costSubtotals.map((subtotal) => <span key={subtotal.currency} className="rounded bg-neutral-950 px-3 py-2 font-mono">{subtotal.amount} {subtotal.currency}</span>)}</div><p className="mt-2 text-xs text-neutral-500">{t("copy.currencySubtotalsRemainSeparateLineageContextIs")}</p></div></Panel>

        <div className="grid gap-6 xl:grid-cols-3">
          <Panel title={t("copy.budgetReservations")} error={errors.budget}><ul className="mt-3 space-y-2 text-sm">{budgetReservations.map((item) => <li key={item.id} className="rounded bg-neutral-950 p-3">{item.state} {t("copy.reserved")} {item.reservedAmount} {item.currency} {t("copy.held")} {item.heldAmount} {t("copy.settled")} {item.settledAmount} {t("copy.released")} {item.releasedAmount}</li>)}</ul></Panel>
          <Panel title={t("copy.quotaReservations")} error={errors.quota}><p className="mt-3 text-xs text-neutral-500">{t("copy.showingUpTo100RowsThisCapability")}</p><ul className="mt-3 space-y-2 text-sm">{quotaReservations.map((item) => <li key={item.id} className="rounded bg-neutral-950 p-3">{item.state} · {item.dimension} {t("copy.reserved")} {item.reservedAmount} {item.unit} {t("copy.held")} {item.heldAmount} {t("copy.settled")} {item.settledAmount} {t("copy.released")} {item.releasedAmount} {t("copy.overage")} {item.overageAmount}</li>)}</ul></Panel>
          <Panel title={t("copy.quotaWaits")} error={errors.waits}><p className="mt-3 text-xs text-neutral-500">{t("copy.showingUpTo100RowsThisCapability")}</p><ul className="mt-3 space-y-2 text-sm">{quotaWaits.map((item) => <li key={item.id} className="rounded bg-neutral-950 p-3">{item.state} {t("copy.eligible")} {display(item.eligibleAt)}</li>)}</ul></Panel>
        </div>

        <Panel title={t("copy.orderedRetainedRunEvents")} error={errors.events}><ol className="mt-4 space-y-2">{events.map((event) => <li key={event.id} className="border-s border-neutral-700 ps-3 text-sm"><strong>#{event.sequence}</strong> {event.type}<time className="ms-2 text-xs text-neutral-500">{event.occurredAt}</time></li>)}</ol><div className="mt-4 flex gap-3"><button type="button" onClick={() => void continueEvents()} className="rounded border border-neutral-700 px-3 py-2 text-sm">{t("copy.checkForRetainedUpdates")}</button>{eventPageFull ? <button type="button" onClick={() => void continueEvents()} className="rounded border border-sky-800 px-3 py-2 text-sm text-sky-200">{t("copy.continueEventHistory")}</button> : null}</div></Panel>

        <Panel title={t("copy.sanitizedDiagnostics")} error={errors.trace}><form onSubmit={inspectTrace} className="mt-4 grid gap-3 md:grid-cols-[1fr_auto_1fr_auto]"><label className="text-sm">{t("copy.operatorTraceReference")}<select name="operatorTraceRef" required value={traceRef ?? ""} onChange={(event) => setTraceRef(event.target.value)} className="mt-1 w-full rounded border border-neutral-700 bg-neutral-950 px-3 py-2"><option value="">{t("copy.selectReference")}</option>{traceReferences.map((ref) => <option key={ref}>{ref}</option>)}</select></label><button type="button" disabled={!traceRef} onClick={() => void copyTraceReference()} className="self-end rounded border border-neutral-700 px-3 py-2 text-sm disabled:opacity-50">{t("copy.copyTraceReference")}</button><label className="text-sm">{t("copy.activeTraceReadGrantID")}<input name="operatorGrantId" required className="mt-1 w-full rounded border border-neutral-700 bg-neutral-950 px-3 py-2" /></label><button className="self-end rounded bg-sky-400 px-4 py-2 font-medium text-neutral-950">{t("copy.openSanitizedTrace")}</button></form>{trace ? <dl className="mt-4 grid gap-2 rounded-lg bg-neutral-950 p-4 text-sm md:grid-cols-2">{TRACE_FIELDS.map((key) => <div key={key}><dt className="text-xs text-neutral-500">{key}</dt><dd>{display(trace[key])}</dd></div>)}</dl> : <p className="mt-3 text-sm text-neutral-500">{t("copy.onlyAllowlistedTraceFieldsAreShownRaw")}</p>}</Panel>
      </div>
    </main>
  );
}
