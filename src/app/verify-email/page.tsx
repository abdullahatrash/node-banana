  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-neutral-950 px-4 text-neutral-100">
      ...
      <section className="w-full max-w-md rounded-xl border border-neutral-800 bg-neutral-900 p-6">
        <p className="text-sm text-emerald-300">الخطوة التالية · Next step</p>
        <h1 className="mt-2 text-2xl font-semibold">تحقّق من بريدك الإلكتروني</h1>
        <p className="mt-2 text-sm text-neutral-400">
          أرسلنا رابط التأكيد إلى بريدك. افتح الرابط للمتابعة إلى إعداد مساحة العمل.
        </p>
        <p className="mt-2 text-sm text-neutral-400">
          We sent you a verification link. Open it to continue setting up your workspace.
        </p>

        <label className="mt-6 block">
          <span className="text-xs text-neutral-400">البريد الإلكتروني · Email</span>
          <input
            type="email"
            autoComplete="email"
            value={email}
        ...
        >
          {status === "sending" ? "جارٍ الإرسال…" : "إعادة إرسال الرابط · Resend link"}
        </button>

        {status === "sent" ? (
          <p className="mt-3 text-sm text-emerald-300">تم إرسال رابط جديد.</p>
        ) : null}
        {status === "error" ? (
          <p className="mt-3 text-sm text-red-300">تعذر الإرسال الآن. حاول مرة أخرى.</p>
        ) : null}
