...
Drag the text directly on the preview to move it; double-click or focus it and press Enter to edit in place. Enter finishes editing. Arrow keys move a selected overlay, and Shift+arrow moves it farther. Text stays inside the canvas, and export snapshots the same text and normalized position. The side text field stays synchronized. The prototype supports one line of text; longer text scales down to fit. Preview and export share the same font, text raster drawing, and geometry through `overlay.mjs`.
