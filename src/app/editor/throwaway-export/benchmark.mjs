...
try{
  ...
  if(process.env.PROTOTYPE_CHECK_CANCEL==='1'){
    ...
    await page.waitForFunction(()=>document.getElementById('progress').value>1);
    ...
    const cancelled={status:'cancelled',cancelMs:Date.now()-start,remainingTemporaryOutputs:remaining};
  ...
  for(const entry of cases){
    ...
    for(;;){
      ...
      if(state){
        const record={...state,hardware,sessionRssBaselineBytes:memory[0],sessionRssPeakBytes:Math.max(...memory),rssNote:'Sum of browser-session process RSS, including shared pages; not unique/private memory.',memorySamples:memory.length};
        records.push(record);
