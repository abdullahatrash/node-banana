...
execFileSync('ffmpeg',['-v','error','-y','-i',`${root}/marker-44100.wav`,'-c:a','libmp3lame','-b:a','128k',`${root}/marker-44100.mp3`]);
...
try{
 ...
 const results=[];
 for(const [rate,extension] of [[48000,'wav'],[44100,'wav'],[44100,'mp3']]){
  await page.locator('#voiceFile').setInputFiles(`${root}/marker-${rate}.${extension}`);await page.waitForFunction(()=>document.getElementById('voice').readyState>=1);
  ...
  const record={rate,extension,...result,remainingTemporaryAudio};results.push(record);
  if(result.status==='complete'){const dl=page.waitForEvent('download');await page.locator('#download').click();const path=`${root}/mixed-${rate}-${extension}.mp4`;await(await dl).saveAs(path);record.alignment=alignment(path);}
 }
 ...
 await writeFile(`${root}/results.json`,JSON.stringify({results,direct},null,2));console.log(JSON.stringify({results:results.map(r=>({rate:r.rate,extension:r.extension,status:r.status,message:r.message,end:r.audioLastPacketEnd,normalized:r.normalizedAudioSources,alignment:r.alignment,remainingTemporaryAudio:r.remainingTemporaryAudio})),direct}));
