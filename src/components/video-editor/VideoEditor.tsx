...
export function VideoEditor({ locale = 'ar', initialId }: { locale?: 'ar' | 'en'; initialId?: string }) {
 ...
 const open = useCallback(async (record: EditorRecord) => {
  const items = record.composition.main ? [record.composition.main.assetId] : [];
 ...
 async function select(item: MediaItem) {
  ...
  try { const selected = await resolveMedia(item); if (selected.duration <= 0 || Math.max(selected.width, selected.height) > 1920 || Math.min(selected.width, selected.height) > 1080) throw new Error('EDITOR_MEDIA_LIMIT'); setMedia((items) => ({ ...items, [item.id]: selected })); setComposition((value) => ({ ...value, main: createClip(item.id, selected.duration) })); }
 ...
 return <main className={styles.editor} lang={locale} dir={locale === 'ar' ? 'rtl' : 'ltr'}>
  ...
  <div className={styles.workspace} dir="ltr">
   ...
   <aside className={styles.panel} dir={locale === 'ar' ? 'rtl' : 'ltr'}><h2>{copy.main}</h2>
    {composition.main && <>{(['trimStart', 'trimEnd'] as const).map((field) => <label key={field}>{field === 'trimStart' ? copy.start : copy.end}<input type="number" step={1 / 30} min={0} max={media[composition.main!.assetId]?.duration || 60} value={composition.main![field]} onChange={(e) => setComposition({ ...composition, main: { ...composition.main!, [field]: Number(e.target.value) } })} /></label>)}</>}
   </aside>
  </div>
  <section className={styles.timeline} dir="ltr"><span>{copy.main}</span><div className={styles.clip}>{composition.main ? media[composition.main.assetId]?.name : '—'} <small>{duration(composition).toFixed(1)}s</small></div></section>
