...
beforeEach(() => {
 ...
 vi.stubGlobal('fetch', vi.fn(async (url: string, init?: RequestInit) => {
  ...
  if (url.startsWith('/api/video-editor')) return Response.json({ success: true, records: saved });
...
it('undoes a grouped edit and preserves it after a stale-revision save fails', async () => {
 ...
 fireEvent.click(screen.getByRole('button', { name: 'Save' }));
 await screen.findByText('Saved');
 fireEvent.change(screen.getByLabelText('Trim end'), { target: { value: '4' } });
 fireEvent.click(screen.getByRole('button', { name: 'Undo' }));
 expect(screen.getByLabelText('Trim end')).toHaveValue(5);
 fireEvent.click(screen.getByRole('button', { name: 'Redo' }));
 const original = vi.mocked(fetch).getMockImplementation()!;
 vi.mocked(fetch).mockImplementation(async (url, init) => String(url) === '/api/video-editor' && init?.method === 'POST' ? Response.json({ success: false, code: 'EDITOR_SAVE_CONFLICT' }, { status: 409 }) : original(url, init));
 fireEvent.click(screen.getByRole('button', { name: 'Save' }));
 await screen.findByRole('alert');
...
it('retries an uncertain save with the same key and stays in its original Workspace', async () => {
 ...
 vi.mocked(fetch).mockImplementation(async (url, init) => {
  if (String(url) === '/api/video-editor' && init?.method === 'POST' && first) { first = false; throw new Error('connection lost'); }
 ...
 fireEvent.click(screen.getByRole('button', { name: 'Save' }));
 await screen.findByText('Saved');
 const writes = vi.mocked(fetch).mock.calls.filter(([url, init]) => url === '/api/video-editor' && init?.method === 'POST');
