...
export function TextControls({
  ...
  value: TextOverlay;
  onChange(value: TextOverlay | null): void;
  copy: EditorCopy;
}) {
  return (
    <>
        ...
        <select
          ...
          onChange={(event) =>
            onChange({
              ...
              fontWeight: Number(
                event.target.value,
              ) as TextOverlay["fontWeight"],
            })
          }
        >
          {([100, 300, 400, 700] as const).map((weight) => (
      ...
      <label>
        ...
        <input
          ...
          onChange={(event) =>
            onChange({ ...value, backgroundColor: event.target.value })
          }
        />
      </label>
      <label>
      ...
      <p>{copy.textHelp}</p>
      <button onClick={() => onChange(null)}>{copy.remove}</button>
    </>
  );
}
