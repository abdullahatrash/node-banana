...
export function ServiceStatusBanner() {
  ...
  useEffect(() => { const controller = new AbortController(); void fetch(`/api/status?locale=${locale}`, { cache: "no-store", signal: controller.signal }).then(async (response) => { const data = await response.json() as { configured?: boolean; status?: string; incidents?: Array<{ summary?: string }> }; if (data.configured === false) { setStatus(null); setSummary(""); return; } setStatus(data.status === "operational" || data.status === "degraded" || data.status === "majorOutage" || data.status === "criticalOutage" ? data.status : "unknown"); setSummary(data.incidents?.[0]?.summary || ""); }).catch(() => setStatus("unknown")); return () => controller.abort(); }, [locale]);
