...
ALTER TABLE "product_telemetry_events" ADD CONSTRAINT "product_telemetry_events_workspace_pseudonym_check" CHECK (
  ...
  (
    ("subject_pseudonym" IS NULL AND "region_classification" IS NULL AND "expires_at" IS NULL) OR
    ("subject_pseudonym" ~ '^sub_[a-f0-9]{64}$' AND "region_classification" IN ('mena','non_mena','unknown') AND "expires_at" > "received_at")
  ) AND
